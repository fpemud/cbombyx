...
# NM options
parser.add_argument('--nm', dest='do_set_byx_logging', action='store',
...
if args.do_set_byx_logging:
    #print args.log_domains
    dom_msg = ""
    if args.log_domains:
            dom_msg = " for domains: " + ','.join(args.log_domains)
    print("Setting NetworkManager log level to '" + args.do_set_byx_logging + "'" + dom_msg)
    ...
    nm.SetLogging(args.do_set_byx_logging, ','.join(args.log_domains))

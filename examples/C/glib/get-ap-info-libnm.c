...
show_access_point_info (NMAccessPoint *ap)
{
	...
	/* Convert to strings */
	if (ssid)
		ssid_str = byx_utils_ssid_to_utf8 (g_bytes_get_data (ssid, NULL), g_bytes_get_size (ssid));
	else
		ssid_str = g_strdup ("--");
...
show_wifi_device_info (NMDevice *device)
{
	...
	if (nm_device_get_state (device) == NM_DEVICE_STATE_ACTIVATED) {
		if ((active_ap = nm_device_wifi_get_active_access_point (NM_DEVICE_WIFI (device)))) {
			...
			if (active_ssid)
				active_ssid_str = byx_utils_ssid_to_utf8 (g_bytes_get_data (active_ssid, NULL),

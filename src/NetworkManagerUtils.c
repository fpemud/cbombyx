                         ...
                         const char *fallback_prefix)
{
	...
	for (i = 1; TRUE; i++) {
		...
		if (byx_utils_strv_find_first ((char **) existing_names,
		                              existing_len,
		                              temp) < 0)
			return temp;
...
const char *
byx_utils_get_ip_config_method (NMConnection *connection,
...
gboolean
byx_utils_connection_has_default_route (NMConnection *connection,
                                       int addr_family,
...
void
byx_utils_complete_generic (NMPlatform *platform,
                           ...
                           gboolean default_enable_ipv6)
{
	...
	if (!nm_setting_connection_get_uuid (s_con)) {
		...
		g_object_set (G_OBJECT (s_con), NM_SETTING_CONNECTION_UUID, byx_utils_uuid_generate_buf (uuid), NULL);
	}
                  ...
                  GHashTable *settings)
{
	 ...
	 */
	orig_ip6_method = byx_utils_get_ip_config_method (orig, NM_TYPE_SETTING_IP6_CONFIG);
	candidate_ip6_method = byx_utils_get_ip_config_method (candidate, NM_TYPE_SETTING_IP6_CONFIG);
...
route_compare (NMIPRoute *route1, NMIPRoute *route2, gint64 default_metric)
{
	...
	byx_utils_ipx_address_clear_host_address (family, &a1, &a1, plen);
	byx_utils_ipx_address_clear_host_address (family, &a2, &a2, plen);
                  ...
                  gboolean device_has_carrier)
{
	 ...
	 */
	orig_ip4_method = byx_utils_get_ip_config_method (orig, NM_TYPE_SETTING_IP4_CONFIG);
	candidate_ip4_method = byx_utils_get_ip_config_method (candidate, NM_TYPE_SETTING_IP4_CONFIG);
 ...
 */
NMConnection *
byx_utils_match_connection (NMConnection *const*connections,
                           ...
                           gpointer match_filter_data)
{
	...
	for (; *connections; connections++) {
		...
		if (indicated) {
			...
			if (!best_match && byx_logging_enabled (LOGL_DEBUG, LOGD_CORE)) {
				GString *diff_string;
				...
				byx_log_dbg (LOGD_CORE, "Connection '%s' differs from candidate '%s' in %s",

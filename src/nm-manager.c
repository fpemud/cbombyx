...
NM_GOBJECT_PROPERTIES_DEFINE (ByxManager,
	PROP_VERSION,
	PROP_CAPABILITIES,
	PROP_STATE,
	PROP_STARTUP,
...
G_DEFINE_TYPE (ByxManager, nm_manager, NM_TYPE_DBUS_OBJECT)

#define BYX_MANAGER_GET_PRIVATE(self) _BYX_GET_PRIVATE(self, ByxManager, BYX_IS_MANAGER)

/*****************************************************************************/

BYX_DEFINE_SINGLETON_INSTANCE (ByxManager);
...
#define _NMLOG(level, domain, ...) \
    G_STMT_START { \
        const ByxLogLevel _level = (level); \
        const NMLogDomain _domain = (domain); \
        \
        if (byx_logging_enabled (_level, _domain)) { \
            const ByxManager *const _self = (self); \
            char _sbuf[32]; \
            \
            _byx_log (_level, _domain, 0, NULL, NULL, \
...
static const ByxDBusInterfaceInfoExtended interface_info_manager;
...
static gboolean add_device (ByxManager *self, NMDevice *device, GError **error);

static void _emit_device_added_removed (ByxManager *self,
                                        NMDevice *device,
                                        gboolean is_added);

static NMActiveConnection *_new_active_connection (ByxManager *self,
                                                   gboolean is_vpn,
                                                   NMConnection *connection,
...
static gboolean find_master (ByxManager *self,
                             NMConnection *connection,
                             NMDevice *device,
...
static void byx_manager_update_state (ByxManager *manager);

static void connection_changed (ByxManager *self, NMConnection *connection);
...
static void retry_connections_for_parent_device (ByxManager *self, NMDevice *device);
...
static NMActiveConnection *active_connection_find (ByxManager *self,
                                                   NMSettingsConnection *settings_connection,
                                                   const char *uuid,
                                                   NMActiveConnectionState max_state,
                                                   GPtrArray **out_all_matching);

static NMConnectivity *concheck_get_mgr (ByxManager *self);

static void _internal_activation_auth_done (ByxManager *self,
                                            NMActiveConnection *active,
                                            gboolean success,
                                            const char *error_desc);
static void _add_and_activate_auth_done (ByxManager *self,
                                         NMActiveConnection *active,
                                         NMConnection *connection,
                                         GDBusMethodInvocation *invocation,
                                         gboolean success,
                                         const char *error_desc);
static void _activation_auth_done (ByxManager *self,
                                   NMActiveConnection *active,
                                   GDBusMethodInvocation *invocation,
                                   gboolean success,
                                   const char *error_desc);
...
static gboolean
concheck_enabled (ByxManager *self, gboolean *out_changed)
{
...
concheck_get_mgr (ByxManager *self)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
static AsyncOpData *
_async_op_data_new_authorize_activate_internal (ByxManager *self, NMActiveConnection *active_take)
{
	...
	c_list_link_tail (&BYX_MANAGER_GET_PRIVATE (self)->async_op_lst_head, &async_op_data->async_op_lst);
	return async_op_data;
}

static AsyncOpData *
_async_op_data_new_ac_auth_activate_user (ByxManager *self,
...
static AsyncOpData *
_async_op_data_new_ac_auth_add_and_activate (ByxManager *self,
                                             NMActiveConnection *active_take,
                                             GDBusMethodInvocation *invocation_take,
                               ...
                               gpointer user_data)
{
	...
	nm_assert (BYX_IS_MANAGER (async_op_data->self));
	nm_assert (nm_c_list_contains_entry (&BYX_MANAGER_GET_PRIVATE (async_op_data->self)->async_op_lst_head, async_op_data, async_op_lst));
...
static guint32
_device_route_metric_get (ByxManager *self,
                          int ifindex,
                          NMDeviceType device_type,
                          gboolean lookup_only,
                          guint32 *out_aspired_metric)
{
	ByxManagerPrivate *priv;
	...
	g_return_val_if_fail (BYX_IS_MANAGER (self), 0);
	...
	priv = BYX_MANAGER_GET_PRIVATE (self);
...
guint32
byx_manager_device_route_metric_reserve (ByxManager *self,
                                        int ifindex,
                                        NMDeviceType device_type)
{
	guint32 metric;
...
void
byx_manager_device_route_metric_clear (ByxManager *self,
                                      int ifindex)
{
	ByxManagerPrivate *priv;
	DeviceRouteMetricData data_lookup;
...
static void
_delete_volatile_connection_do (ByxManager *self,
...
static gboolean
active_connection_remove (ByxManager *self, NMActiveConnection *active)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
 ...
 */
static void
active_connection_add (ByxManager *self,
                       NMActiveConnection *active)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
const CList *
byx_manager_get_active_connections (ByxManager *manager)
{
	return &BYX_MANAGER_GET_PRIVATE (manager)->active_connections_lst_head;
}

static NMActiveConnection *
active_connection_find (ByxManager *self,
                        NMSettingsConnection *settings_connection,
                        const char *uuid,
...
static NMActiveConnection *
active_connection_find_by_connection (ByxManager *self,
                                      NMConnection *connection,
                                      NMActiveConnectionState max_state,
                                      GPtrArray **out_all_matching)
{
...
byx_manager_get_activatable_connections (ByxManager *manager, guint *out_len, gboolean sort)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (manager);
...
static NMActiveConnection *
active_connection_get_by_path (ByxManager *self, const char *path)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMActiveConnection *ac;

	ac = (NMActiveConnection *) byx_dbus_manager_lookup_object (nm_dbus_object_get_manager (NM_DBUS_OBJECT (self)),
	                                                           path);
	if (   !ac
...
NMDevice *
byx_manager_get_device_by_path (ByxManager *self, const char *path)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMDevice *device;
	...
	device = (NMDevice *) byx_dbus_manager_lookup_object (nm_dbus_object_get_manager (NM_DBUS_OBJECT (self)),
	                                                     path);
	if (   !device
	    || !NM_IS_DEVICE (device)
...
NMDevice *
byx_manager_get_device_by_ifindex (ByxManager *self, int ifindex)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMDevice *device;
...
static NMDevice *
find_device_by_permanent_hw_addr (ByxManager *self, const char *hwaddr)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMDevice *device;
	...
	if (!_byx_utils_hwaddr_aton (hwaddr, hwaddr_bin, sizeof (hwaddr_bin), &hwaddr_len))
		return NULL;

	c_list_for_each_entry (device, &priv->devices_lst_head, devices_lst) {
		device_addr = nm_device_get_permanent_hw_address (device);
		if (   device_addr
		    && byx_utils_hwaddr_matches (hwaddr_bin, hwaddr_len, device_addr, -1))
			return device;
	}
	return NULL;
}
...
static NMDevice *
find_device_by_ip_iface (ByxManager *self, const gchar *iface)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMDevice *device;
 ...
 */
static NMDevice *
find_device_by_iface (ByxManager *self,
                      const char *iface,
                      NMConnection *connection,
                      NMConnection *slave)
{
...
static gboolean
manager_sleeping (ByxManager *self)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
static NMState
find_best_device_state (ByxManager *manager)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (manager);
...
static void
byx_manager_update_metered (ByxManager *self)
{
	ByxManagerPrivate *priv;
	NMDevice *device;
	NMMetered value = NM_METERED_UNKNOWN;

	g_return_if_fail (BYX_IS_MANAGER (self));
	priv = BYX_MANAGER_GET_PRIVATE (self);
...
static void
byx_manager_update_state (ByxManager *self)
{
	ByxManagerPrivate *priv;
...
static void
check_if_startup_complete (ByxManager *self)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMDevice *device;
...
static void
_parent_notify_changed (ByxManager *self,
                        NMDevice *device,
...
{
	remove_device (BYX_MANAGER (user_data), device, FALSE, TRUE);
}

NMState
byx_manager_get_state (ByxManager *manager)
{
	g_return_val_if_fail (BYX_IS_MANAGER (manager), NM_STATE_UNKNOWN);

	return BYX_MANAGER_GET_PRIVATE (manager)->state;
}

/*****************************************************************************/

static NMDevice *
find_parent_device_for_connection (ByxManager *self, NMConnection *connection, ByxDeviceFactory *cached_factory)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	...
	if (!cached_factory) {
		factory = byx_device_factory_manager_find_factory_for_connection (connection);
		if (!factory)
			return NULL;
	} else
		factory = cached_factory;

	parent_name = byx_device_factory_get_connection_parent (factory, connection);
	if (!parent_name)
		return NULL;
...
/**
 * byx_manager_get_connection_iface:
 * @self: the #ByxManager
 ...
 */
char *
byx_manager_get_connection_iface (ByxManager *self,
                                 NMConnection *connection,
                                 NMDevice **out_parent,
                                 GError **error)
{
 ...
 */
const char *
byx_manager_iface_for_uuid (ByxManager *self, const char *uuid)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMSettingsConnection *connection;
...
NMDevice *
byx_manager_get_device (ByxManager *self, const char *ifname, NMDeviceType device_type)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMDevice *device;
...
gboolean
byx_manager_remove_device (ByxManager *self, const char *ifname, NMDeviceType device_type)
{
	NMDevice *d;

	d = byx_manager_get_device (self, ifname, device_type);
	if (!d)
		return FALSE;
 ...
 */
static NMDevice *
system_create_virtual_device (ByxManager *self, NMConnection *connection)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	ByxDeviceFactory *factory;
	...
	GError *error = NULL;
	ByxLogLevel log_level;

	g_return_val_if_fail (BYX_IS_MANAGER (self), NULL);
	g_return_val_if_fail (NM_IS_CONNECTION (connection), NULL);

	iface = byx_manager_get_connection_iface (self, connection, &parent, &error);
	if (!iface) {
	...
	if (!device) {
		...
		device = byx_device_factory_create_device (factory, iface, NULL, connection, NULL, &error);
		if (!device) {
		...
		/* Add device takes a reference that ByxManager still owns, so it's
...
static void
retry_connections_for_parent_device (ByxManager *self, NMDevice *device)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	gs_free NMSettingsConnection **connections = NULL;
	guint i;
	...
	for (i = 0; connections[i]; i++) {
		...
		if (parent == device) {
			...
			ifname = byx_manager_get_connection_iface (self, candidate, &parent, &error);
			if (ifname) {
...
static void
connection_changed (ByxManager *self,
                    NMConnection *connection)
{
	NMDevice *device;
...
static gboolean
radio_enabled_for_type (ByxManager *self, RfKillType rtype, gboolean check_changeable)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
static void
manager_update_radio_enabled (ByxManager *self,
                              RadioState *rstate,
                              gboolean enabled)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
static void
manager_rfkill_update_one_type (ByxManager *self,
                                RadioState *rstate,
                                RfKillType rtype)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
static void
byx_manager_rfkill_update (ByxManager *self, RfKillType rtype)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	guint i;
...
get_existing_connection (ByxManager *self,
                         NMDevice *device,
                         gboolean *out_generated)
{
	...
	if (ifindex) {
		...
		if (master_ifindex) {
			master = byx_manager_get_device_by_ifindex (self, master_ifindex);
			if (!master) {
	...
	if (   assume_state_connection_uuid
		...
		if (connection) {
			...
			matched = NM_SETTINGS_CONNECTION (byx_utils_match_connection (connections,
			                                                             connection,
			                                                             TRUE,
	...
	if (!matched && assume_state_guess_assume) {
		...
		connections = byx_manager_get_activatable_connections (self, &len, FALSE);
		if (len > 0) {
			for (i = 0, j = 0; i < len; i++) {
			...
			matched = NM_SETTINGS_CONNECTION (byx_utils_match_connection ((NMConnection *const*) connections,
			                                                             connection,
			                                                             FALSE,
			                                                             nm_device_has_carrier (device),
			                                                             nm_device_get_route_metric (device, AF_INET),
...
static gboolean
recheck_assume_connection (ByxManager *self,
                           NMDevice *device)
{
	NMSettingsConnection *connection;
	...
	g_return_val_if_fail (BYX_IS_MANAGER (self), FALSE);
...
static void
recheck_assume_connection_cb (ByxManager *self, NMDevice *device)
{
...
static void
_emit_device_added_removed (ByxManager *self,
                            NMDevice *device,
                            gboolean is_added)
{
	nm_dbus_object_emit_signal (NM_DBUS_OBJECT (self),
...
static void
_device_realize_finish (ByxManager *self,
                        NMDevice *device,
 ...
 */
static gboolean
add_device (ByxManager *self, NMDevice *device, GError **error)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	...
	if (ifindex > 0 && byx_manager_get_device_by_ifindex (self, ifindex)) {
	...
	if (rtype != RFKILL_TYPE_UNKNOWN) {
		byx_manager_rfkill_update (self, rtype);
...
static void
factory_device_added_cb (ByxDeviceFactory *factory,
                         NMDevice *device,
                         gpointer user_data)
{
...
static gboolean
factory_component_added_cb (ByxDeviceFactory *factory,
                            GObject *component,
                            gpointer user_data)
{
	ByxManager *self = user_data;
...
static void
_register_device_factory (ByxDeviceFactory *factory, gpointer user_data)
{
	ByxManager *self = BYX_MANAGER (user_data);

	g_signal_connect (factory,
	                  BYX_DEVICE_FACTORY_DEVICE_ADDED,
	                  G_CALLBACK (factory_device_added_cb),
	                  self);
	g_signal_connect (factory,
	                  BYX_DEVICE_FACTORY_COMPONENT_ADDED,
...
static void
platform_link_added (ByxManager *self,
                     int ifindex,
                     const NMPlatformLink *plink,
                     ...
                     const ByxConfigDeviceStateData *dev_state)
{
	...
	if (byx_manager_get_device_by_ifindex (self, ifindex))
		return;
	...
	factory = byx_device_factory_manager_find_factory_for_link_type (plink->type);
	if (factory) {
		gboolean ignore = FALSE;
		gs_free_error GError *error = NULL;

		device = byx_device_factory_create_device (factory, plink->name, plink, NULL, &ignore, &error);
		if (!device) {
			if (!ignore) {
...
_platform_link_cb_idle (PlatformLinkCbData *data)
{
	...
	if (plink) {
		...
		device = byx_manager_get_device_by_ifindex (self, ifindex);
		if (device) {
...
static void
platform_query_devices (ByxManager *self)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
rfkill_manager_rfkill_changed_cb (ByxRfkillManager *rfkill_mgr,
                                  ...
                                  gpointer user_data)
{
	byx_manager_rfkill_update (BYX_MANAGER (user_data), rtype);
}

const CList *
byx_manager_get_devices (ByxManager *manager)
{
	g_return_val_if_fail (BYX_IS_MANAGER (manager), NULL);

	return &BYX_MANAGER_GET_PRIVATE (manager)->devices_lst_head;
}

static NMDevice *
byx_manager_get_best_device_for_connection (ByxManager *self,
                                           NMConnection *connection,
...
static const char **
_get_devices_paths (ByxManager *self,
...
static void
impl_manager_get_devices (ByxDBusObject *obj,
                          const ByxDBusInterfaceInfoExtended *interface_info,
                          const NMDBusMethodInfoExtended *method_info,
                          GDBusConnection *connection,
                          const char *sender,
...
static void
impl_manager_get_all_devices (ByxDBusObject *obj,
                              const ByxDBusInterfaceInfoExtended *interface_info,
                              const NMDBusMethodInfoExtended *method_info,
                              GDBusConnection *connection,
                              const char *sender,
...
static void
impl_manager_get_device_by_ip_iface (ByxDBusObject *obj,
                                     const ByxDBusInterfaceInfoExtended *interface_info,
                                     const NMDBusMethodInfoExtended *method_info,
                                     GDBusConnection *connection,
                                     const char *sender,
                                     ...
                                     GVariant *parameters)
{
	...
	if (!path) {
		g_dbus_method_invocation_return_error (invocation,
		                                       BYX_MANAGER_ERROR,
		                                       BYX_MANAGER_ERROR_UNKNOWN_DEVICE,
             ...
             GError **error)
{
	...
	if (master_device) {
		...
		if (master_connection && !is_compatible_with_slave (NM_CONNECTION (master_connection), connection)) {
			g_set_error (error, BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_DEPENDENCY_FAILED,
	...
	if (master_device || master_connection)
		...
		g_set_error_literal (error, BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_UNKNOWN_DEVICE,
 ...
 */
static NMActiveConnection *
ensure_master_active_connection (ByxManager *self,
                                 NMAuthSubject *subject,
                                 NMConnection *connection,
                                 NMDevice *device,
                                 ...
                                 GError **error)
{
	...
	if (master_device) {
		...
		if (master_state == NM_DEVICE_STATE_DISCONNECTED || !nm_device_is_real (master_device)) {
			...
			connections = byx_manager_get_activatable_connections (self, NULL, TRUE);
			for (i = 0; connections[i]; i++) {
				...
				if (nm_device_check_connection_available (master_device, NM_CONNECTION (candidate), NM_DEVICE_CHECK_CON_AVAILABLE_FOR_USER_REQUEST, NULL)) {
					master_ac = byx_manager_activate_connection (self,
					                                            candidate,
					                                            NULL,
					                                            NULL,
					                                            master_device,
			...
			g_set_error (error,
			             BYX_MANAGER_ERROR,
			             BYX_MANAGER_ERROR_UNKNOWN_CONNECTION,
		...
		g_set_error (error,
		             BYX_MANAGER_ERROR,
		             BYX_MANAGER_ERROR_DEPENDENCY_FAILED,
...
find_slaves (ByxManager *manager,
             NMSettingsConnection *connection,
             NMDevice *device,
             guint *out_n_slaves)
{
	...
	for (i = 0; i < n_all_connections; i++) {
		...
		if (   (master_connection && master_connection == connection)
			...
			slave_device = byx_manager_get_best_device_for_connection (manager,
			                                                          candidate,
			                                                          FALSE,
			                                                          devices);
...
static void
autoconnect_slaves (ByxManager *self,
                    NMSettingsConnection *master_connection,
                    NMDevice *master_device,
                    NMAuthSubject *subject)
{
	...
	if (should_connect_slaves (NM_CONNECTION (master_connection), master_device)) {
		...
		for (i = 0; i < n_slaves; i++) {
			...
			byx_manager_activate_connection (self,
			                                slave->connection,
			                                NULL,
			                                NULL,
			                                slave->device,
...
static gboolean
_internal_activate_vpn (ByxManager *self, NMActiveConnection *active, GError **error)
{
	nm_assert (NM_IS_VPN_CONNECTION (active));

	nm_dbus_object_export (NM_DBUS_OBJECT (active));
	if (!nm_service_manager_activate_service (BYX_MANAGER_GET_PRIVATE (self)->service_manager,
...
static gboolean
_internal_activate_device (ByxManager *self, NMActiveConnection *active, GError **error)
{
	...
	if (!nm_device_is_real (device)) {
		...
		if (parent && !nm_device_is_real (parent)) {
			...
			parent_ac = byx_manager_activate_connection (self, parent_con, NULL, NULL, parent,
			                                            subject,
			                                            NM_ACTIVATION_TYPE_MANAGED,
...
static gboolean
_internal_activate_generic (ByxManager *self, NMActiveConnection *active, GError **error)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
static NMActiveConnection *
_new_active_connection (ByxManager *self,
                        gboolean is_vpn,
                        NMConnection *connection,
                        NMConnection *applied,
                        const char *specific_object,
                        ...
                        GError **error)
{
	...
	if (is_vpn) {
		...
		if (active_connection_find_by_connection (self, connection, NM_ACTIVE_CONNECTION_STATE_ACTIVATED, NULL)) {
			g_set_error (error, BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_CONNECTION_ALREADY_ACTIVE,
		...
		if (specific_object) {
			...
			if (!parent) {
				g_set_error_literal (error, BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_CONNECTION_NOT_ACTIVE,
		...
		if (!parent) {
			g_set_error_literal (error, BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_UNKNOWN_CONNECTION,
...
static void
_internal_activation_auth_done (ByxManager *self,
                                NMActiveConnection *active,
                                gboolean success,
                                const char *error_desc)
{
	...
	if (NM_IN_SET (nm_active_connection_get_activation_reason (active), NM_ACTIVATION_REASON_EXTERNAL,
		...
		c_list_for_each_entry (ac, &priv->active_connections_lst_head, active_connections_lst) {
			if (   nm_active_connection_get_device (ac) == nm_active_connection_get_device (active)
				...
				g_set_error (&error,
				             BYX_MANAGER_ERROR,
				             BYX_MANAGER_ERROR_CONNECTION_ALREADY_ACTIVE,
				             "Connection '%s' is already active",
...
/**
 * byx_manager_activate_connection():
 * @self: the #ByxManager
 ...
 */
NMActiveConnection *
byx_manager_activate_connection (ByxManager *self,
                                NMSettingsConnection *connection,
                                NMConnection *applied,
                                const char *specific_object,
                                NMDevice *device,
...
validate_activation_request (ByxManager *self,
                             GDBusMethodInvocation *context,
                             NMConnection *connection,
                             ...
                             GError **error)
{
	...
	if (*out_device) {
		...
		nm_assert (device == byx_manager_get_device_by_path (self, device_path));
	} else if (device_path) {
		device = byx_manager_get_device_by_path (self, device_path);
		if (!device) {
			g_set_error_literal (error,
			                     BYX_MANAGER_ERROR,
			                     BYX_MANAGER_ERROR_UNKNOWN_DEVICE,
		...
		device = byx_manager_get_best_device_for_connection (self, connection, TRUE, NULL);
		if (!device) {
			gs_free char *iface = NULL;
			...
			iface = byx_manager_get_connection_iface (self, connection, NULL, error);
			if (!iface)
				return NULL;
...
static void
_activation_auth_done (ByxManager *self,
                       NMActiveConnection *active,
                       GDBusMethodInvocation *invocation,
                       gboolean success,
                       const char *error_desc)
...
static void
impl_manager_activate_connection (ByxDBusObject *obj,
                                  const ByxDBusInterfaceInfoExtended *interface_info,
                                  const NMDBusMethodInfoExtended *method_info,
                                  ...
                                  GVariant *parameters)
{
	...
	connection_path = byx_utils_dbus_normalize_object_path (connection_path);
	specific_object_path = byx_utils_dbus_normalize_object_path (specific_object_path);
	device_path = byx_utils_dbus_normalize_object_path (device_path);
	...
	if (connection_path) {
		...
		if (!device_path) {
			error = g_error_new_literal (BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_UNKNOWN_DEVICE,
			                             "Only devices may be activated without a specifying a connection");
			goto error;
		}
		device = byx_manager_get_device_by_path (self, device_path);
		if (!device) {
			error = g_error_new (BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_UNKNOWN_DEVICE,
                     ...
                     gpointer user_data)
{
	...
	byx_utils_user_data_unpack (user_data, &self, &active);
...
static void
_add_and_activate_auth_done (ByxManager *self,
                             NMActiveConnection *active,
                             NMConnection *connection,
                             GDBusMethodInvocation *invocation,
                             gboolean success,
                             const char *error_desc)
{
	...
	nm_settings_add_connection_dbus (priv->settings,
	                                 ...
	                                 invocation,
	                                 activation_add_done,
	                                 byx_utils_user_data_pack (self,
	                                                          g_object_ref (active)));
}

static void
impl_manager_add_and_activate_connection (ByxDBusObject *obj,
                                          const ByxDBusInterfaceInfoExtended *interface_info,
                                          const NMDBusMethodInfoExtended *method_info,
                                          GDBusConnection *dbus_connection,
                                          const char *sender,
                                          ...
                                          GVariant *parameters)
{
	 ...
	 * the connection will be completed in byx_utils_complete_generic() or
	...
	if (is_vpn) {
		...
		byx_utils_complete_generic (priv->platform,
		                           connection,
...
gboolean
byx_manager_deactivate_connection (ByxManager *manager,
                                  NMActiveConnection *active,
                                  NMDeviceStateReason reason,
                                  GError **error)
{
                             ...
                             gpointer user_data)
{
	...
	if (auth_error) {
		...
		error = g_error_new_literal (BYX_MANAGER_ERROR,
		                             BYX_MANAGER_ERROR_CONNECTION_NOT_ACTIVE,
		                             "The connection was not active.");
	} else {
		/* success; deactivation allowed */
		if (!byx_manager_deactivate_connection (self,
		                                       active,
		                                       NM_DEVICE_STATE_REASON_USER_REQUESTED,
		                                       &error))
			nm_assert (error);
...
static void
impl_manager_deactivate_connection (ByxDBusObject *obj,
                                    const ByxDBusInterfaceInfoExtended *interface_info,
                                    const NMDBusMethodInfoExtended *method_info,
                                    GDBusConnection *dbus_connection,
                                    const char *sender,
...
static void
sleep_devices_clear (ByxManager *self)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMDevice *device;
	ByxSleepMonitorInhibitorHandle *handle;
...
static void
do_sleep_wake (ByxManager *self, gboolean sleeping_changed)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	...
	if (manager_sleeping (self)) {
		 ...
		 */
		byx_manager_rfkill_update (self, RFKILL_TYPE_UNKNOWN);
...
static void
_internal_sleep (ByxManager *self, gboolean do_sleep)
{
	ByxManagerPrivate *priv;
...
static void
_internal_enable (ByxManager *self, gboolean enable)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
static void
impl_manager_enable (ByxDBusObject *obj,
                     const ByxDBusInterfaceInfoExtended *interface_info,
                     const NMDBusMethodInfoExtended *method_info,
                     GDBusConnection *connection,
                     const char *sender,
                     ...
                     GVariant *parameters)
{
	...
	if (priv->net_enabled == enable) {
		error = g_error_new (BYX_MANAGER_ERROR,
		                     BYX_MANAGER_ERROR_ALREADY_ENABLED_OR_DISABLED,
...
static void
get_perm_add_result (ByxManager *self, NMAuthChain *chain, GVariantBuilder *results, const char *permission)
{
	NMAuthCallResult result;
                    ...
                    GVariant *parameters)
{
	...
	byx_manager_update_state (self);
	g_dbus_method_invocation_return_value (invocation,
	                                       g_variant_new ("(u)", BYX_MANAGER_GET_PRIVATE (self)->state));
}

static void
impl_manager_set_logging (ByxDBusObject *obj,
                          const ByxDBusInterfaceInfoExtended *interface_info,
                          const NMDBusMethodInfoExtended *method_info,
                          GDBusConnection *connection,
                          const char *sender,
                          ...
                          GVariant *parameters)
{
	 ...
	 */
	if (!byx_dbus_manager_ensure_uid (nm_dbus_object_get_manager (NM_DBUS_OBJECT (self)),
	                                 invocation,
	                                 G_MAXULONG,
	                                 BYX_MANAGER_ERROR,
	                                 BYX_MANAGER_ERROR_PERMISSION_DENIED))
	...
	if (byx_logging_setup (level, domains, NULL, &error)) {
		_LOGI (LOGD_CORE, "logging: level '%s' domains '%s'",
		       byx_logging_level_to_string (), byx_logging_domains_to_string ());
	}
...
static void
impl_manager_get_logging (ByxDBusObject *obj,
                          const ByxDBusInterfaceInfoExtended *interface_info,
                          const NMDBusMethodInfoExtended *method_info,
                          GDBusConnection *connection,
                          const char *sender,
                          ...
                          GVariant *parameters)
{
	g_dbus_method_invocation_return_value (invocation,
	                                       g_variant_new ("(ss)",
	                                                      byx_logging_level_to_string (),
	                                                      byx_logging_domains_to_string ()));
}
                          ...
                          gpointer user_data)
{
	...
	nm_assert (BYX_IS_MANAGER (data->self));
...
static void
impl_manager_check_connectivity (ByxDBusObject *obj,
                                 const ByxDBusInterfaceInfoExtended *interface_info,
                                 const NMDBusMethodInfoExtended *method_info,
                                 GDBusConnection *connection,
                                 const char *sender,
...
static void
start_factory (ByxDeviceFactory *factory, gpointer user_data)
{
	byx_device_factory_start (factory);
}
...
gboolean
byx_manager_start (ByxManager *self, GError **error)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	gs_free NMSettingsConnection **connections = NULL;
	guint i;
	...
	byx_device_factory_manager_for_each_factory (start_factory, NULL);
	...
	/* Load VPN plugins */
	priv->service_manager = g_object_ref (byx_service_manager_get ());
...
void
byx_manager_stop (ByxManager *self)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMDevice *device;
	...
	byx_dbus_manager_stop (nm_dbus_object_get_manager (NM_DBUS_OBJECT (self)));
...
connection_metered_changed (GObject *object,
                            ...
                            gpointer user_data)
{
	byx_manager_update_metered (BYX_MANAGER (user_data));
}
...
policy_default_device_changed (GObject *object, GParamSpec *pspec, gpointer user_data)
{
	...
	if (ac != priv->primary_connection) {
		...
		byx_manager_update_metered (self);
	}
}
                            ...
                            gpointer user_data)
{
	...
	gs_unref_object ByxDBusObject *obj = handle_data->obj;
	const ByxDBusInterfaceInfoExtended *interface_info = handle_data->interface_info;
	const ByxDBusPropertyInfoExtended *property_info = handle_data->property_info;
	gs_unref_variant GVariant *value = handle_data->value;
	guint64 export_version_id = handle_data->export_version_id;
	gs_unref_object ByxManager *self = handle_data->self;
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	NMAuthCallResult result;
...
void
byx_manager_dbus_set_property_handle (ByxDBusObject *obj,
                                     const ByxDBusInterfaceInfoExtended *interface_info,
                                     const ByxDBusPropertyInfoExtended *property_info,
                                     GDBusConnection *connection,
                                     const char *sender,
...
static void
rfkill_change (ByxManager *self, const char *desc, RfKillType rtype, gboolean enabled)
{
	int fd;
...
static void
manager_radio_user_toggled (ByxManager *self,
                            RadioState *rstate,
                            gboolean enabled)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
...
{
	ByxManager *manager = BYX_MANAGER (user_data);
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (manager);
	NMActiveConnection *ac;
...
void
byx_manager_set_capability (ByxManager *self,
                           NMCapability cap)
{
	ByxManagerPrivate *priv;
	guint32 cap_i;
	...
	idx = _byx_utils_array_find_binary_search (&g_array_index (priv->capabilities, guint32, 0),
	                                          sizeof (guint32),
...
BYX_DEFINE_SINGLETON_REGISTER (ByxManager);

ByxManager *
byx_manager_get (void)
{
...
nm_settings_get (void)
{
	...
	return BYX_MANAGER_GET_PRIVATE (singleton_instance)->settings;
}

ByxManager *
byx_manager_setup (void)
{
	ByxManager *self;

	g_return_val_if_fail (!singleton_instance, singleton_instance);

	self = g_object_new (BYX_TYPE_MANAGER, NULL);
	nm_assert (BYX_IS_MANAGER (self));
	singleton_instance = self;

	byx_singleton_instance_register ();
	_LOGD (LOGD_CORE, "setup %s singleton (%p)", "ByxManager", singleton_instance);
...
static void
byx_manager_init (ByxManager *self)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE (self);
	guint i;
	GFile *file;
              ...
              const GValue *value, GParamSpec *pspec)
{
	...
	switch (prop_id) {
	case PROP_WIRELESS_ENABLED:
		manager_radio_user_toggled (BYX_MANAGER (object),
...
static void
_deinit_device_factory (ByxDeviceFactory *factory, gpointer user_data)
{
	g_signal_handlers_disconnect_matched (factory, G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL, BYX_MANAGER (user_data));
}
...
dispose (GObject *object)
{
	...
	G_OBJECT_CLASS (byx_manager_parent_class)->dispose (object);
}

static void
finalize (GObject *object)
{
	ByxManagerPrivate *priv = BYX_MANAGER_GET_PRIVATE ((ByxManager *) object);

	g_array_free (priv->capabilities, TRUE);

	G_OBJECT_CLASS (byx_manager_parent_class)->finalize (object);
...
static const ByxDBusInterfaceInfoExtended interface_info_manager = {
	.parent = NM_DEFINE_GDBUS_INTERFACE_INFO_INIT (
		...
		.properties = NM_DEFINE_GDBUS_PROPERTY_INFOS (
			...
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READWRITABLE_L ("WwanEnabled",                "b",     BYX_MANAGER_WWAN_ENABLED,                  NM_AUTH_PERMISSION_ENABLE_DISABLE_WWAN,               NM_AUDIT_OP_RADIO_CONTROL),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("WwanHardwareEnabled",        "b",     BYX_MANAGER_WWAN_HARDWARE_ENABLED),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READWRITABLE_L ("WimaxEnabled",               "b",     BYX_MANAGER_WIMAX_ENABLED,                 NM_AUTH_PERMISSION_ENABLE_DISABLE_WIMAX,              NM_AUDIT_OP_RADIO_CONTROL),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("WimaxHardwareEnabled",       "b",     BYX_MANAGER_WIMAX_HARDWARE_ENABLED),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("ActiveConnections",          "ao",    BYX_MANAGER_ACTIVE_CONNECTIONS),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("PrimaryConnection",          "o",     BYX_MANAGER_PRIMARY_CONNECTION),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("PrimaryConnectionType",      "s",     BYX_MANAGER_PRIMARY_CONNECTION_TYPE),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("Metered",                    "u",     BYX_MANAGER_METERED),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("ActivatingConnection",       "o",     BYX_MANAGER_ACTIVATING_CONNECTION),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("Startup",                    "b",     BYX_MANAGER_STARTUP),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("Version",                    "s",     BYX_MANAGER_VERSION),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("Capabilities",               "u",     BYX_MANAGER_CAPABILITIES),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("State",                      "u",     BYX_MANAGER_STATE),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("Connectivity",               "u",     BYX_MANAGER_CONNECTIVITY),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE_L     ("ConnectivityCheckAvailable", "b",     BYX_MANAGER_CONNECTIVITY_CHECK_AVAILABLE),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READWRITABLE_L ("ConnectivityCheckEnabled",   "b",     BYX_MANAGER_CONNECTIVITY_CHECK_ENABLED,    NM_AUTH_PERMISSION_ENABLE_DISABLE_CONNECTIVITY_CHECK, NM_AUDIT_OP_NET_CONTROL),
			NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READWRITABLE_L ("GlobalDnsConfiguration",     "a{sv}", BYX_MANAGER_GLOBAL_DNS_CONFIGURATION,      NM_AUTH_PERMISSION_SETTINGS_MODIFY_GLOBAL_DNS,        NM_AUDIT_OP_NET_CONTROL),
		),
	),
...
static void
byx_manager_class_init (ByxManagerClass *manager_class)
{
	GObjectClass *object_class = G_OBJECT_CLASS (manager_class);
	ByxDBusObjectClass *dbus_object_class = NM_DBUS_OBJECT_CLASS (manager_class);
	...
	obj_properties[PROP_VERSION] =
	    g_param_spec_string (BYX_MANAGER_VERSION, "", "",
	                         NULL,
	                         G_PARAM_READABLE |
	                         G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_CAPABILITIES] =
	    g_param_spec_variant (BYX_MANAGER_CAPABILITIES, "", "",
	                          G_VARIANT_TYPE ("au"),
	                          NULL,
	                          G_PARAM_READABLE |
	                          G_PARAM_STATIC_STRINGS);
	...
	obj_properties[PROP_STATE] =
	    g_param_spec_uint (BYX_MANAGER_STATE, "", "",
	                       0, NM_STATE_DISCONNECTED, 0,
	                       G_PARAM_READABLE |
	                       G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_STARTUP] =
	    g_param_spec_boolean (BYX_MANAGER_STARTUP, "", "",
	                          TRUE,
	                          G_PARAM_READABLE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_NETWORKING_ENABLED] =
	    g_param_spec_boolean (BYX_MANAGER_NETWORKING_ENABLED, "", "",
	                          TRUE,
	                          G_PARAM_READABLE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_WIRELESS_ENABLED] =
	    g_param_spec_boolean (BYX_MANAGER_WIRELESS_ENABLED, "", "",
	                          TRUE,
	                          G_PARAM_READWRITE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_WIRELESS_HARDWARE_ENABLED] =
	    g_param_spec_boolean (BYX_MANAGER_WIRELESS_HARDWARE_ENABLED, "", "",
	                          TRUE,
	                          G_PARAM_READABLE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_WWAN_ENABLED] =
	    g_param_spec_boolean (BYX_MANAGER_WWAN_ENABLED, "", "",
	                          TRUE,
	                          G_PARAM_READWRITE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_WWAN_HARDWARE_ENABLED] =
	    g_param_spec_boolean (BYX_MANAGER_WWAN_HARDWARE_ENABLED, "", "",
	                          TRUE,
	                          G_PARAM_READABLE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_WIMAX_ENABLED] =
	    g_param_spec_boolean (BYX_MANAGER_WIMAX_ENABLED, "", "",
	                          TRUE,
	                          G_PARAM_READWRITE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_WIMAX_HARDWARE_ENABLED] =
	    g_param_spec_boolean (BYX_MANAGER_WIMAX_HARDWARE_ENABLED, "", "",
	                          TRUE,
	                          G_PARAM_READABLE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_ACTIVE_CONNECTIONS] =
	    g_param_spec_boxed (BYX_MANAGER_ACTIVE_CONNECTIONS, "", "",
	                        G_TYPE_STRV,
	                        G_PARAM_READABLE |
	                        G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_CONNECTIVITY] =
	    g_param_spec_uint (BYX_MANAGER_CONNECTIVITY, "", "",
	                       NM_CONNECTIVITY_UNKNOWN, NM_CONNECTIVITY_FULL, NM_CONNECTIVITY_UNKNOWN,
	                       G_PARAM_READABLE |
	                       G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_CONNECTIVITY_CHECK_AVAILABLE] =
	    g_param_spec_boolean (BYX_MANAGER_CONNECTIVITY_CHECK_AVAILABLE, "", "",
	                          TRUE,
	                          G_PARAM_READABLE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_CONNECTIVITY_CHECK_ENABLED] =
	    g_param_spec_boolean (BYX_MANAGER_CONNECTIVITY_CHECK_ENABLED, "", "",
	                          TRUE,
	                          G_PARAM_READWRITE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_PRIMARY_CONNECTION] =
	    g_param_spec_string (BYX_MANAGER_PRIMARY_CONNECTION, "", "",
	                         NULL,
	                         G_PARAM_READABLE |
	                         G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_PRIMARY_CONNECTION_TYPE] =
	    g_param_spec_string (BYX_MANAGER_PRIMARY_CONNECTION_TYPE, "", "",
	                         NULL,
	                         G_PARAM_READABLE |
	                         G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_ACTIVATING_CONNECTION] =
	    g_param_spec_string (BYX_MANAGER_ACTIVATING_CONNECTION, "", "",
	                         NULL,
	                         G_PARAM_READABLE |
	                         G_PARAM_STATIC_STRINGS);
	...
	obj_properties[PROP_SLEEPING] =
	    g_param_spec_boolean (BYX_MANAGER_SLEEPING, "", "",
	                          FALSE,
	                          G_PARAM_READABLE |
	                          G_PARAM_STATIC_STRINGS);

	obj_properties[PROP_DEVICES] =
	    g_param_spec_boxed (BYX_MANAGER_DEVICES, "", "",
	                        G_TYPE_STRV,
	                        G_PARAM_READABLE |
	                        G_PARAM_STATIC_STRINGS);
	...
	obj_properties[PROP_METERED] =
	    g_param_spec_uint (BYX_MANAGER_METERED, "", "",
	                       0, G_MAXUINT32, NM_METERED_UNKNOWN,
	                       G_PARAM_READABLE |
	                       G_PARAM_STATIC_STRINGS);

	/**
	 * ByxManager:global-dns-configuration:
	 *
	...
	obj_properties[PROP_GLOBAL_DNS_CONFIGURATION] =
	    g_param_spec_variant (BYX_MANAGER_GLOBAL_DNS_CONFIGURATION, "", "",
	                          G_VARIANT_TYPE ("a{sv}"),
	                          NULL,
	                          G_PARAM_READWRITE |
	                          G_PARAM_STATIC_STRINGS);
	...
	signals[DEVICE_ADDED] =
	    g_signal_new (BYX_MANAGER_DEVICE_ADDED,
	...
	signals[INTERNAL_DEVICE_ADDED] =
	    g_signal_new (BYX_MANAGER_INTERNAL_DEVICE_ADDED,
	                  G_OBJECT_CLASS_TYPE (object_class),
	...
	signals[DEVICE_REMOVED] =
	    g_signal_new (BYX_MANAGER_DEVICE_REMOVED,
	                  G_OBJECT_CLASS_TYPE (object_class),
	                  G_SIGNAL_RUN_FIRST,
	                  0, NULL, NULL, NULL,
	                  G_TYPE_NONE, 1, NM_TYPE_DEVICE);
	...
	signals[INTERNAL_DEVICE_REMOVED] =
	    g_signal_new (BYX_MANAGER_INTERNAL_DEVICE_REMOVED,
	                  G_OBJECT_CLASS_TYPE (object_class),
	                  G_SIGNAL_RUN_FIRST, 0,
	                  NULL, NULL, NULL,
	                  G_TYPE_NONE, 1, G_TYPE_OBJECT);
	...
	signals[ACTIVE_CONNECTION_ADDED] =
	    g_signal_new (BYX_MANAGER_ACTIVE_CONNECTION_ADDED,
	                  G_OBJECT_CLASS_TYPE (object_class),
	                  G_SIGNAL_RUN_FIRST,
	                  0, NULL, NULL, NULL,
	...
	signals[ACTIVE_CONNECTION_REMOVED] =
	    g_signal_new (BYX_MANAGER_ACTIVE_CONNECTION_REMOVED,
	                  G_OBJECT_CLASS_TYPE (object_class),
	                  G_SIGNAL_RUN_FIRST,
	                  0, NULL, NULL, NULL,
	                  G_TYPE_NONE, 1, NM_TYPE_ACTIVE_CONNECTION);

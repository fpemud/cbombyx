...
#define NM_ACT_REQUEST_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMActRequest, NM_IS_ACT_REQUEST)
...
_do_cancel_secrets (NMActRequest *self, NMActRequestGetSecretsCallId *call_id, gboolean is_disposing)
{
	...
	if (call_id->callback) {
		...
		byx_utils_error_set_cancelled (&error, is_disposing, "NMActRequest");
...
nm_act_request_set_shared (NMActRequest *req, gboolean shared)
{
	...
	for (iter = list; iter; iter = g_slist_next (iter)) {
		...
		if (argv && argv[0]) {
			...
			byx_log_info (LOGD_SHARING, "Executing: %s", cmd);
			if (!g_spawn_sync ("/", argv, envp, G_SPAWN_STDOUT_TO_DEV_NULL | G_SPAWN_STDERR_TO_DEV_NULL,
			                   NULL, NULL, NULL, NULL, &status, &error)) {
				byx_log_warn (LOGD_SHARING, "Error executing command: %s",
				             error->message);
				g_clear_error (&error);
			} else if (WEXITSTATUS (status)) {
				byx_log_warn (LOGD_SHARING, "** Command returned exit status %d.",
				             WEXITSTATUS (status));
			}
		}
	}

...
#define NM_DNS_DNSMASQ_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMDnsDnsmasq, NM_IS_DNS_DNSMASQ)
...
ip_addr_to_string (int addr_family, gconstpointer addr, const char *iface, char *out_buf)
{
	...
	if (addr_family == AF_INET) {
		byx_utils_inet_ntop (addr_family, addr, buf2);
		separator = "@";
	} else {
		if (IN6_IS_ADDR_V4MAPPED (addr))
			byx_utils_inet4_ntop (((const struct in6_addr *) addr)->s6_addr32[3], buf2);
		else
			byx_utils_inet6_ntop (addr, buf2);
...
add_ip_config (NMDnsDnsmasq *self, GVariantBuilder *servers, const NMDnsIPConfigData *ip_data)
{
	...
	for (i = 0; i < num; i++) {
		...
		for (j = 0; ip_data->domains.search[j]; j++) {
			domain = byx_utils_parse_dns_domain (ip_data->domains.search[j], NULL);
...
start_dnsmasq (NMDnsDnsmasq *self)
{
	...
	dm_binary = byx_utils_find_helper ("dnsmasq", DNSMASQ_PATH, NULL);
	if (!dm_binary) {
	...
	dbus_mgr = byx_dbus_manager_get ();
	g_return_if_fail (dbus_mgr);

	connection = byx_dbus_manager_get_connection (dbus_mgr);
	...
	if (WIFEXITED (status)) {
		...
		if (err) {
			_LOGW ("dnsmasq exited with error: %s",
			       byx_utils_dnsmasq_status_to_string (err, NULL, 0));
		} else {

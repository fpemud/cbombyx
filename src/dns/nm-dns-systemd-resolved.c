...
#define NM_DNS_SYSTEMD_RESOLVED_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMDnsSystemdResolved, NM_IS_DNS_SYSTEMD_RESOLVED)
                      ...
                      NMDnsIPConfigData *data)
{
	...
	addr_size = byx_utils_addr_family_to_size (addr_family);
	...
	for (iter = data->domains.search; *iter; iter++) {
		domain = byx_utils_parse_dns_domain (*iter, &is_routing);
        ...
        const char *hostname)
{
	...
	interfaces_keys = byx_utils_hash_keys_to_array (interfaces,

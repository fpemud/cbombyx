...
{
	if (_byx_utils_dns_option_find_idx (array, str) < 0)
                 ...
                 gboolean include_routing, gboolean dup)
{
	...
	for (i = 0; i < num_searches; i++) {
		...
		if (!include_routing && domain_is_routing (str))
			continue;
		if (!domain_is_valid (byx_utils_parse_dns_domain (str, NULL), FALSE))
			continue;
                     ...
                     const NMIPConfig *ip_config)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (addr_family == AF_INET)
			byx_utils_inet_ntop (addr_family, addr, buf);
		else if (IN6_IS_ADDR_V4MAPPED (addr))
			byx_utils_inet4_ntop (addr->addr6.s6_addr32[3], buf);
		else {
			byx_utils_inet6_ntop (&addr->addr6, buf);
	...
	if (addr_family == AF_INET) {
		...
		for (i = 0; i < num; i++) {
			add_string_item (rc->nis_servers,
			                 byx_utils_inet4_ntop (nm_ip4_config_get_nis_server (ip4_config, i), buf),
			                 TRUE);
		}
	...
	if (   rc_manager == NM_DNS_MANAGER_RESOLV_CONF_MAN_FILE
		...
		if (rc_manager == NM_DNS_MANAGER_RESOLV_CONF_MAN_FILE) {
			...
			if (rc_path_real)
				...
				rc_path_syml = byx_utils_read_link_absolute (_PATH_RESCONF, NULL);
				if (rc_path_syml)
...
get_nameserver_list (const NMIPConfig *config, GString **str)
{
	...
	for (i = 0; i < num; i++) {
		byx_utils_inet_ntop (addr_family,
                           ...
                           const char **out_nis_domain)
{
		...
		c_list_for_each_entry (ip_data, head, ip_config_lst) {
			...
			if (nm_ip_config_get_num_nameservers (ip_data->ip_config)) {
				_LOGT ("config: %8d %-7s v%c %-5d %s: %s",
				       ...
				       byx_utils_addr_family_to_char (nm_ip_config_get_addr_family (ip_data->ip_config)),
	...
	if (priv->hostname) {
		...
		if (   hostdomain
		    && !byx_utils_ipaddr_valid (AF_UNSPEC, priv->hostname)) {
			hostdomain++;
...
get_ip_rdns_domains (NMIPConfig *ip_config)
{
	...
	return _byx_utils_strv_cleanup (strv, FALSE, FALSE, TRUE);
}
...
rebuild_domain_lists (NMDnsManager *self)
{
	...
	c_list_for_each_entry (ip_data, head, ip_config_lst) {
		...
		for (i = 0; i < n_domains; i++) {
			...
			domain_clean = byx_utils_parse_dns_domain (domains[i], NULL);
                             ...
                             gboolean skip_update)
{
	...
	if (   hostname
	    && byx_utils_is_specific_hostname (hostname)
...
_resolvconf_resolved_managed (void)
{
	...
	if (S_ISLNK (st.st_mode)) {
		...
		if (byx_utils_strv_find_first ((char **) RESOLVED_PATHS,

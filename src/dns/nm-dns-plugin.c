...
#define NM_DNS_PLUGIN_GET_PRIVATE(self) _BYX_GET_PRIVATE_PTR (self, NMDnsPlugin, NM_IS_DNS_PLUGIN)
...
kill_existing (const char *progname, const char *pidfile, const char *kill_match)
{
	...
	pid = _byx_utils_ascii_str_to_int64 (contents, 10, 2, INT_MAX, -1);
	if (pid == -1)
		goto out;

	start_time = byx_utils_get_start_time_for_pid (pid, NULL, NULL);
	if (start_time == 0)
		goto out;
	...
	byx_utils_kill_process_sync (pid, start_time, SIGKILL, _NMLOG_DOMAIN,
...
nm_dns_plugin_child_kill (NMDnsPlugin *self)
{
	...
	if (priv->pid) {
		byx_utils_kill_child_sync (priv->pid, SIGTERM, _NMLOG_DOMAIN,

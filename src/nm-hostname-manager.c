...
#define NM_HOSTNAME_MANAGER_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMHostnameManager, NM_IS_HOSTNAME_MANAGER)

BYX_DEFINE_SINGLETON_GETTER (NMHostnameManager, nm_hostname_manager_get, NM_TYPE_HOSTNAME_MANAGER);
...
nm_hostname_manager_write_hostname (NMHostnameManager *self, const char *hostname)
{
	...
	if (   lstat (file, &file_stat) == 0
	    ...
	    && (link_path = byx_utils_read_link_absolute (file, NULL)))
		file = link_path;
...
setup_hostname_file_monitors (NMHostnameManager *self)
{
	...
	if (   lstat(path, &file_stat) == 0
	    && S_ISLNK (file_stat.st_mode)
	    && (link_path = byx_utils_read_link_absolute (path, NULL))) {
		path = link_path;

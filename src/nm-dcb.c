...
run_helper (char **argv, guint which, gpointer user_data, GError **error)
{
	...
	helper_path = byx_utils_find_helper ((which == DCBTOOL) ? "dcbtool" : "fcoeadm", NULL, error);
	if (!helper_path)
		return FALSE;

	argv[0] = (char *) helper_path;
	cmdline = g_strjoinv (" ", argv);
	byx_log_dbg (LOGD_DCB, "%s", cmdline);
	...
	if (success && WIFEXITED (exit_status) && WEXITSTATUS (exit_status) && (errmsg || outmsg)) {
		...
		if (ignore_error == FALSE) {
			byx_log_warn (LOGD_DCB, "'%s' failed: '%s'",
...
carrier_wait (const char *iface, guint secs, gboolean up)
{
	...
	if (ifindex > 0) {
		 ...
		 */
		byx_log_dbg (LOGD_DCB, "(%s): cleanup waiting for carrier %s",
		            iface, up ? "up" : "down");

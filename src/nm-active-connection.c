...
#define NM_ACTIVE_CONNECTION_GET_PRIVATE(self) _BYX_GET_PRIVATE_PTR(self, NMActiveConnection, NM_IS_ACTIVE_CONNECTION)

/*****************************************************************************/

static const ByxDBusInterfaceInfoExtended interface_info_active_connection;
                                           ...
                                           NMActivationStateFlags mask)
{
	...
	if (f != priv->state_flags) {
		char buf1[G_N_ELEMENTS (_byx_utils_to_string_buffer)];
		char buf2[G_N_ELEMENTS (_byx_utils_to_string_buffer)];
                                              ...
                                              NMSettingsConnection *connection)
{
	 ...
	 * never changes (once it's set). That has effects for ByxService and
	 * NMActivationRequest.
                                ...
                                gpointer user_data)
{
	...
	wifi_permission = byx_utils_get_shared_wifi_permission (con);
	if (wifi_permission) {
              ...
              const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_SPECIFIC_OBJECT:
		...
		tmp = byx_utils_dbus_normalize_object_path (tmp);
...
nm_active_connection_class_init (NMActiveConnectionClass *ac_class)
{
	...
	ByxDBusObjectClass *dbus_object_class = NM_DBUS_OBJECT_CLASS (ac_class);

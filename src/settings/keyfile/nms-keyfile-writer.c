             ...
             GError **error)
{
	...
	if (scheme == NM_SETTING_802_1X_CK_SCHEME_PATH) {
		...
		success = byx_utils_file_set_contents (new_path, (const gchar *) blob_data,
		                                      blob_len, 0600, &local);
		if (success) {
			...
			byx_log_warn (LOGD_SETTINGS, "keyfile: %s.%s: failed to write certificate to file %s: %s",
                            ...
                            GError **error)
{
	...
	byx_utils_file_set_contents (path, data, len, 0600, &local_err);
	if (local_err) {
		g_set_error (error, NM_SETTINGS_ERROR, NM_SETTINGS_ERROR_FAILED,

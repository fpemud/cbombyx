...
#define NM_SECRET_AGENT_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMSecretAgent, NM_IS_SECRET_AGENT)
...
cancel_done (GObject *proxy, GAsyncResult *result, gpointer user_data)
{
	...
	if (!ret) {
		byx_log_dbg (LOGD_AGENTS, "%s%s%s: agent failed to cancel secrets: %s",
...
do_cancel_secrets (NMSecretAgent *self, NMSecretAgentCallId *r, gboolean disposing)
{
	...
	if (callback) {
		...
		byx_utils_error_set_cancelled (&error, disposing, "NMSecretAgent");
...
static void
_on_disconnected_private_connection (ByxDBusManager *mgr,
                                     GDBusConnection *connection,
                                     NMSecretAgent *self)
{
	NMSecretAgentPrivate *priv = NM_SECRET_AGENT_GET_PRIVATE (self);
                     ...
                     NMSecretAgentCapabilities capabilities)
{
	...
	priv->bus_mgr = g_object_ref (byx_dbus_manager_get ());
	priv->connection = g_object_ref (connection);
	priv->connection_is_private = !!byx_dbus_manager_connection_get_private_name (priv->bus_mgr, connection);
	...
	priv->proxy = byx_dbus_manager_new_proxy (priv->bus_mgr,
	                                         priv->connection,
	                                         G_TYPE_DBUS_PROXY,
	                                         priv->dbus_owner,
	...
	if (priv->connection_is_private) {
		priv->on_disconnected_id = g_signal_connect (priv->bus_mgr,
		                                             BYX_DBUS_MANAGER_PRIVATE_CONNECTION_DISCONNECTED,

...
#define NM_AGENT_MANAGER_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMAgentManager, NM_IS_AGENT_MANAGER)

/*****************************************************************************/

BYX_DEFINE_SINGLETON_GETTER (NMAgentManager, nm_agent_manager_get, NM_TYPE_AGENT_MANAGER);
...
#define _NMLOG(level, agent, ...) \
    G_STMT_START { \
        if (byx_logging_enabled ((level), (_NMLOG_DOMAIN))) { \
            char __prefix1[32]; \
            char __prefix2[128]; \
            ...
            if (__agent) { \
                ...
                __prefix2[0] = '\0'; \
            _byx_log ((level), (_NMLOG_DOMAIN), 0, NULL, NULL, \
...
static void
impl_agent_manager_register (ByxDBusObject *obj,
...
static void
impl_agent_manager_register_with_capabilities (ByxDBusObject *obj,
                                               const ByxDBusInterfaceInfoExtended *interface_info,
                                               const NMDBusMethodInfoExtended *method_info,
                                               GDBusConnection *connection,
                                               const char *sender,
...
static void
impl_agent_manager_unregister (ByxDBusObject *obj,
                               const ByxDBusInterfaceInfoExtended *interface_info,
                               const NMDBusMethodInfoExtended *method_info,
                               GDBusConnection *connection,
                               const char *sender,
...
req_complete_cancel (Request *req, gboolean is_disposing)
{
	...
	byx_utils_error_set_cancelled (&error, is_disposing, "NMAgentManager");
...
static const ByxDBusInterfaceInfoExtended interface_info_agent_manager = {
...
nm_agent_manager_class_init (NMAgentManagerClass *agent_manager_class)
{
	...
	ByxDBusObjectClass *dbus_object_class = NM_DBUS_OBJECT_CLASS (agent_manager_class);

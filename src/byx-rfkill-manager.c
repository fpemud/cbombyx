...
sysfs_state_to_nm_state (gint sysfs_state)
{
	...
	default:
		byx_log_warn (LOGD_RFKILL, "unhandled rfkill state %d", sysfs_state);
		break;
	}
	return RFKILL_UNBLOCKED;
}
...
recheck_killswitches (ByxRfkillManager *self)
{
	...
	for (iter = priv->killswitches; iter; iter = g_slist_next (iter)) {
		...
		if (!device)
			continue;
		sysfs_state = _byx_utils_ascii_str_to_int64 (udev_device_get_property_value (device, "RFKILL_STATE"),
		                                            10, G_MININT, G_MAXINT, -1);
		dev_state = sysfs_state_to_nm_state (sysfs_state);

		byx_log_dbg (LOGD_RFKILL, "%s rfkill%s switch %s state now %d/%u",
	...
	for (i = 0; i < RFKILL_TYPE_MAX; i++) {
		...
		if (poll_states[i] != priv->rfkill_states[i]) {
			byx_log_dbg (LOGD_RFKILL, "%s rfkill state now '%s'",
...
add_one_killswitch (ByxRfkillManager *self, struct udev_device *device)
{
	...
	byx_log_info (LOGD_RFKILL, "%s: found %s radio killswitch (at %s) (%sdriver %s)",
	             ks->name,
               ...
               struct udev_device *device)
{
	...
	for (iter = priv->killswitches; iter; iter = g_slist_next (iter)) {
		...
		if (!strcmp (ks->name, name)) {
			byx_log_info (LOGD_RFKILL, "radio killswitch %s disappeared", ks->path);
               ...
               gpointer user_data)
{
	...
	byx_log_dbg (LOGD_PLATFORM, "udev rfkill event: action '%s' device '%s'",

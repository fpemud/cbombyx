...
#define NM_CONNECTIVITY_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMConnectivity, NM_IS_CONNECTIVITY)

BYX_DEFINE_SINGLETON_GETTER (NMConnectivity, nm_connectivity_get, NM_TYPE_CONNECTIVITY);
...
#define _NMLOG2(level, ...) \
    G_STMT_START { \
        ...
        \
        if (byx_logging_enabled (__level, _NMLOG2_DOMAIN)) { \
            _byx_log (__level, _NMLOG2_DOMAIN, 0, \
...
nm_connectivity_check_cancel (NMConnectivityCheckHandle *cb_data)
{
	...
	byx_utils_error_set_cancelled (&error, FALSE, "NMConnectivity");
...
again:
	c_list_for_each_entry (cb_data, &priv->handles_lst_head, handles_lst) {
		if (!error)
			byx_utils_error_set_cancelled (&error, TRUE, "NMConnectivity");

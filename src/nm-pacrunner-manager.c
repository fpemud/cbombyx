...
#define NM_PACRUNNER_MANAGER_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMPacrunnerManager, NM_IS_PACRUNNER_MANAGER)

/*****************************************************************************/

BYX_DEFINE_SINGLETON_GETTER (NMPacrunnerManager, nm_pacrunner_manager_get, NM_TYPE_PACRUNNER_MANAGER);
...
#define _NMLOG2(level, config, ...) \
	G_STMT_START { \
		byx_log ((level), _NMLOG_DOMAIN, NULL, NULL, \
...
get_ip4_domains (GPtrArray *domains, NMIP4Config *ip4)
{
	...
	nm_ip_config_iter_ip4_route_for_each (&ipconf_iter, ip4, &routes) {
		if (NM_PLATFORM_IP_ROUTE_IS_DEFAULT (routes))
			continue;
		cidr = g_strdup_printf ("%s/%u",
		                        byx_utils_inet4_ntop (routes->network, NULL),
		                        routes->plen);
		g_ptr_array_add (domains, cidr);
	}
}
...
get_ip6_domains (GPtrArray *domains, NMIP6Config *ip6)
{
	...
	nm_ip_config_iter_ip6_route_for_each (&ipconf_iter, ip6, &routes) {
		if (NM_PLATFORM_IP_ROUTE_IS_DEFAULT (routes))
			continue;
		cidr = g_strdup_printf ("%s/%u",
		                        byx_utils_inet6_ntop (&routes->network, NULL),
		                        routes->plen);
		g_ptr_array_add (domains, cidr);
	}
}

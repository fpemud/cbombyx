...
/* Internal VPN states, private to ByxService */
typedef enum {
	STATE_UNKNOWN = 0,
	STATE_WAITING,
	STATE_PREPARE,
...
NM_GOBJECT_PROPERTIES_DEFINE (ByxService,
	PROP_VPN_STATE,
	PROP_BANNER,
...
static NMSettingsConnection *_get_settings_connection (ByxService *self,
                                                       gboolean allow_missing);

static void get_secrets (ByxService *self,
                         SecretsReq secrets_idx,
                         const char *const*hints);

static guint32 get_route_table (ByxService *self, int addr_family, gboolean fallback_main);

static void plugin_interactive_secrets_required (ByxService *self,
                                                 const char *message,
                                                 const char *const*secrets);

static void _set_vpn_state (ByxService *self,
                            VpnState vpn_state,
...
static const char *
__LOG_create_prefix (char *buf, ByxService *self, NMConnection *con)
{
...
#define _NMLOG(level, ...) \
    G_STMT_START { \
        ...
        if (byx_logging_enabled (__level, _NMLOG_DOMAIN)) { \
            ...
            \
            _byx_log (__level, _NMLOG_DOMAIN, 0, \
...
static void
cancel_get_secrets (ByxService *self)
{
...
_get_settings_connection (ByxService *self, gboolean allow_missing)
{
...
static NMConnection *
_get_applied_connection (ByxService *connection)
{
	NMConnection *con;
...
static void
fw_call_cleanup (ByxService *self)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
static void
remove_parent_device_config (ByxService *connection, NMDevice *device)
{
...
static void
vpn_cleanup (ByxService *self, NMDevice *parent_dev)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
{
	ByxService *self = NM_VPN_CONNECTION (user_data);
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
static void
dispatcher_cleanup (ByxService *self)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
static void
_set_vpn_state (ByxService *self,
                VpnState vpn_state,
                NMActiveConnectionStateReason reason,
                gboolean quitting)
{
...
static gboolean
_service_and_connection_can_persist (ByxService *self)
{
...
static gboolean
_connection_only_can_persist (ByxService *self)
{
	return NM_VPN_CONNECTION_GET_PRIVATE (self)->connection_can_persist &&
	       !NM_VPN_CONNECTION_GET_PRIVATE (self)->service_can_persist;
}
	...
	if (nm_platform_ip_route_get (platform,
		...
		if (r->ifindex == ifindex) {
			...
			if (   nm_platform_route_table_is_main (r->table_coerced)
			    && (   r->gateway
			        || byx_utils_ip_is_site_local (AF_INET, &vpn_gw))) {
				parent_gw = r->gateway;
				has_parent_gw = TRUE;
			}
		}
		...
		if (r->ifindex == ifindex) {
			...
			if (   nm_platform_route_table_is_main (r->table_coerced)
			    && (   !IN6_IS_ADDR_UNSPECIFIED (&r->gateway)
			        || byx_utils_ip_is_site_local (AF_INET6, &vpn_gw))) {
				parent_gw = &r->gateway;
				has_parent_gw = TRUE;
			}
		}
                       ...
                       NMAuthSubject *subject)
{
	...
	return (ByxService *) g_object_new (NM_TYPE_VPN_CONNECTION,
...
const char *
nm_vpn_connection_get_service (ByxService *self)
{
	NMSettingVpn *s_vpn;
...
static void
plugin_failed (ByxService *self, guint reason)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
static void
plugin_state_changed (ByxService *self, NMVpnServiceState new_service_state)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
static void
print_vpn_config (ByxService *self)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
	...
	if (priv->ip4_external_gw) {
		_LOGI ("Data: VPN Gateway: %s",
		       byx_utils_inet4_ntop (priv->ip4_external_gw, NULL));
	} else if (priv->ip6_external_gw) {
		_LOGI ("Data: VPN Gateway: %s",
		       byx_utils_inet6_ntop (priv->ip6_external_gw, NULL));
	}
	...
	if (priv->ip4_config) {
		...
		if (priv->ip4_internal_gw)
			_LOGI ("Data:   Internal Gateway: %s", byx_utils_inet4_ntop (priv->ip4_internal_gw, NULL));
		_LOGI ("Data:   Internal Address: %s", address4 ? byx_utils_inet4_ntop (address4->address, NULL) : "??");
		_LOGI ("Data:   Internal Prefix: %d", address4 ? (int) address4->plen : -1);
		_LOGI ("Data:   Internal Point-to-Point Address: %s", byx_utils_inet4_ntop (address4->peer_address, NULL));

		nm_ip_config_iter_ip4_route_for_each (&ipconf_iter, priv->ip4_config, &route) {
			_LOGI ("Data:   Static Route: %s/%d   Next Hop: %s",
			       byx_utils_inet4_ntop (route->network, NULL),
			       route->plen,
			       byx_utils_inet4_ntop (route->gateway, buf));
		}

		num = nm_ip4_config_get_num_nameservers (priv->ip4_config);
		for (i = 0; i < num; i++) {
			_LOGI ("Data:   Internal DNS: %s",
			       byx_utils_inet4_ntop (nm_ip4_config_get_nameserver (priv->ip4_config, i), NULL));
		}
	...
	if (priv->ip6_config) {
		...
		if (priv->ip6_internal_gw)
			_LOGI ("Data:   Internal Gateway: %s", byx_utils_inet6_ntop (priv->ip6_internal_gw, NULL));
		_LOGI ("Data:   Internal Address: %s", byx_utils_inet6_ntop (&address6->address, NULL));
		_LOGI ("Data:   Internal Prefix: %d", address6->plen);
		_LOGI ("Data:   Internal Point-to-Point Address: %s", byx_utils_inet6_ntop (&address6->peer_address, NULL));

		nm_ip_config_iter_ip6_route_for_each (&ipconf_iter, priv->ip6_config, &route) {
			_LOGI ("Data:   Static Route: %s/%d   Next Hop: %s",
			       byx_utils_inet6_ntop (&route->network, NULL),
			       route->plen,
			       byx_utils_inet6_ntop (&route->gateway, buf));
		}

		num = nm_ip6_config_get_num_nameservers (priv->ip6_config);
		for (i = 0; i < num; i++) {
			_LOGI ("Data:   Internal DNS: %s",
			       byx_utils_inet6_ntop (nm_ip6_config_get_nameserver (priv->ip6_config, i), NULL));
		}
...
static void
apply_parent_device_config (ByxService *self)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
	NMDevice *parent_dev = nm_active_connection_get_device (NM_ACTIVE_CONNECTION (self));
	int ifindex;
...
static gboolean
nm_vpn_connection_apply_config (ByxService *self)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
static void
_cleanup_failed_config (ByxService *self)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
static void
nm_vpn_connection_config_maybe_complete (ByxService *self,
                                         gboolean         success)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
static gboolean
process_generic_config (ByxService *self, GVariant *dict)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
	const char *str;
	GVariant *v;
...
static void
nm_vpn_connection_config_get (ByxService *self, GVariant *dict)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
	gboolean b;
...
guint32
nm_vpn_connection_get_ip4_route_metric (ByxService *self)
{
	gint64 route_metric;
	NMConnection *applied;
...
guint32
nm_vpn_connection_get_ip6_route_metric (ByxService *self)
{
	gint64 route_metric;
	NMConnection *applied;
...
static guint32
get_route_table (ByxService *self,
                 int addr_family,
...
static void
nm_vpn_connection_ip4_config_get (ByxService *self, GVariant *dict)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
		...
		while (g_variant_iter_next (iter, "@au", &v)) {
			...
			case 4:
				...
				if (plen > 32 || plen == 0)
					break;
				route.plen = plen;
				route.network = byx_utils_ip4_address_clear_host_address (route.network, plen);
...
static void
nm_vpn_connection_ip6_config_get (ByxService *self, GVariant *dict)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
static void
connect_success (ByxService *self)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
	NMSettingVpn *s_vpn;
	guint32 timeout;
...
static void
really_activate (ByxService *self, const char *username)
{
	NMVpnConnectionPrivate *priv;
...
{
	ByxService *self = NM_VPN_CONNECTION (data);
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
_get_log_level (void)
{
	 ...
	 * maps ByxLogLevel differently to the syslog levels then we
	 * do here.
	 *
	...
	level = byx_logging_get_level (LOGD_VPN_PLUGIN);
	if (level != _LOGL_OFF) {
		if (level <= LOGL_TRACE)
			return LOG_DEBUG;
		if (level <= LOGL_DEBUG)
...
static gboolean
nm_vpn_service_daemon_exec (ByxService *self, GError **error)
{
	NMVpnConnectionPrivate *priv;
	GPid pid;
	char *vpn_argv[4];
	...
	envp[i++] = nm_sprintf_buf (env_log_syslog, "NM_VPN_LOG_SYSLOG=%c", byx_logging_syslog_enabled () ? '1' : '0');

	envp[i++] = NULL;
	nm_assert (i <= n_environ + N_ENVIRON_EXTRA);

	success = g_spawn_async (NULL, vpn_argv, envp, 0, byx_utils_setpgid, NULL, &pid, &spawn_error);
	...
	if (success) {
		...
		g_set_error (error,
		             BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_FAILED,
...
void
nm_vpn_connection_activate (ByxService *self,
                            NMVpnPluginInfo *plugin_info)
{
	NMVpnConnectionPrivate *priv;
	NMSettingVpn *s_vpn;
...
nm_vpn_connection_get_vpn_state (ByxService *self)
{
...
const char *
nm_vpn_connection_get_banner (ByxService *self)
{
...
NMProxyConfig *
nm_vpn_connection_get_proxy_config (ByxService *self)
{
	g_return_val_if_fail (NM_IS_VPN_CONNECTION (self), NULL);
...
NMIP4Config *
nm_vpn_connection_get_ip4_config (ByxService *self)
{
	g_return_val_if_fail (NM_IS_VPN_CONNECTION (self), NULL);
...
NMIP6Config *
nm_vpn_connection_get_ip6_config (ByxService *self)
{
	g_return_val_if_fail (NM_IS_VPN_CONNECTION (self), NULL);
...
static int
_get_ip_iface_for_device (ByxService *self, const char **out_iface)
{
	NMDevice *parent_dev;
	int ifindex;
	const char *iface;
...
const char *
nm_vpn_connection_get_ip_iface (ByxService *self, gboolean fallback_device)
{
	NMVpnConnectionPrivate *priv;
	const char *iface;
...
int
nm_vpn_connection_get_ip_ifindex (ByxService *self, gboolean fallback_device)
{
	NMVpnConnectionPrivate *priv;
...
guint32
nm_vpn_connection_get_ip4_internal_gateway (ByxService *self)
{
	g_return_val_if_fail (NM_IS_VPN_CONNECTION (self), 0);
...
struct in6_addr *
nm_vpn_connection_get_ip6_internal_gateway (ByxService *self)
{
	g_return_val_if_fail (NM_IS_VPN_CONNECTION (self), 0);
...
void
nm_vpn_connection_disconnect (ByxService *self,
                              NMActiveConnectionStateReason reason,
                              gboolean quitting)
{
	g_return_if_fail (NM_IS_VPN_CONNECTION (self));
...
gboolean
nm_vpn_connection_deactivate (ByxService *self,
                              NMActiveConnectionStateReason reason,
                              gboolean quitting)
{
	NMVpnConnectionPrivate *priv;
...
static void
plugin_interactive_secrets_required (ByxService *self,
                                     const char *message,
                                     const char *const*secrets)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
device_changed (NMActiveConnection *active,
                ...
                NMDevice *old_device)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE ((ByxService *) active);
...
static void
nm_vpn_connection_init (ByxService *self)
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
{
	ByxService *self = NM_VPN_CONNECTION (object);
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE (self);
...
{
	NMVpnConnectionPrivate *priv = NM_VPN_CONNECTION_GET_PRIVATE ((ByxService *) object);
...
nm_vpn_connection_class_init (NMVpnConnectionClass *connection_class)
{
	...
	ByxDBusObjectClass *dbus_object_class = NM_DBUS_OBJECT_CLASS (connection_class);

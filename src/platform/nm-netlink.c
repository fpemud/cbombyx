...
nl_nlmsghdr_to_str (const struct nlmsghdr *hdr, char *buf, gsize len)
{
	...
	if (!byx_utils_to_string_buffer_init_null (hdr, &buf, &len))
		return buf;
	...
	if (s)
		byx_utils_strbuf_append_str (&buf, &len, s);
	else
		byx_utils_strbuf_append (&buf, &len, "(%u)", (unsigned) hdr->nlmsg_type);

	flags = hdr->nlmsg_flags;

	if (!flags) {
		byx_utils_strbuf_append_str (&buf, &len, ", flags 0");
		goto flags_done;
	}
...
#define _F(f, n) \
	G_STMT_START { \
		if (NM_FLAGS_ALL (flags, f)) { \
			flags &= ~(f); \
			byx_utils_strbuf_append (&buf, &len, "%s%s", prefix, n); \
			if (!flags) \
				goto flags_done; \
			prefix = ","; \
		} \
...
#undef _F
	...
	if (flags_before != flags)
		prefix = ";";
	byx_utils_strbuf_append (&buf, &len, "%s0x%04x", prefix, flags);

flags_done:

	byx_utils_strbuf_append (&buf, &len, ", seq %u", (unsigned) hdr->nlmsg_seq);

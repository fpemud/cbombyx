...
__ns_types_to_str (int ns_types, int ns_types_already_set, char *buf, gsize len)
{
	...
	byx_utils_strbuf_append_c (&buf, &len, '[');
...
#define _NMLOG(level, netns, ...) \
    G_STMT_START { \
        ByxLogLevel _level = (level); \
        \
        if (byx_logging_enabled (_level, _NMLOG_DOMAIN)) { \
...
#define NMP_NETNS_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMPNetns, NMP_IS_NETNS)

...
ethtool_get (int ifindex, gpointer edata)
{
	...
	if (!nmp_utils_if_indextoname (ifindex, ifname)) {
		byx_log_trace (LOGD_PLATFORM, "ethtool[%d]: %s: request fails resolving ifindex: %s",
		              ifindex,
...

	{
		...
		if (fd < 0) {
			byx_log_trace (LOGD_PLATFORM, "ethtool[%d]: %s, %s: failed creating socket for ioctl: %s",
			              ifindex,
			              _ethtool_data_to_string (edata, sbuf, sizeof (sbuf)),
			              ifname,
			              g_strerror (errno));
		...
		if (ioctl (fd, SIOCETHTOOL, &ifr) < 0) {
			byx_log_trace (LOGD_PLATFORM, "ethtool[%d]: %s, %s: failed: %s",
			              ifindex,
			              _ethtool_data_to_string (edata, sbuf, sizeof (sbuf)),
			              ifname,
			              strerror (errno));
		...
		byx_log_trace (LOGD_PLATFORM, "ethtool[%d]: %s, %s: success",
		              ifindex,
		              _ethtool_data_to_string (edata, sbuf, sizeof (sbuf)),
		              ifname);
		return TRUE;
...
nmp_utils_ethtool_supports_vlans (int ifindex)
{
	...
	if (idx == -1) {
		byx_log_dbg (LOGD_PLATFORM, "ethtool: vlan-challenged ethtool feature does not exist for %d?", ifindex);
		return FALSE;
	}
...
nmp_utils_ethtool_get_peer_ifindex (int ifindex)
{
	...
	if (peer_ifindex_stat == -1) {
		byx_log_dbg (LOGD_PLATFORM, "ethtool: peer_ifindex stat for %d does not exist?", ifindex);
		return FALSE;
	}
                                   ...
                                   const char *wol_password)
{
	...
	byx_log_dbg (LOGD_PLATFORM, "setting Wake-on-LAN options 0x%x, password '%s'",
	...
	if (wol_password) {
		if (!byx_utils_hwaddr_aton (wol_password, wol_info.sopass, ETH_ALEN)) {
			byx_log_dbg (LOGD_PLATFORM, "couldn't parse Wake-on-LAN password '%s'", wol_password);
			return FALSE;
		}
...
nmp_utils_mii_supports_carrier_detect (int ifindex)
{
	...
	if (!nmp_utils_if_indextoname (ifindex, ifname)) {
		byx_log_trace (LOGD_PLATFORM, "mii[%d]: carrier-detect no: request fails resolving ifindex: %s", ifindex, g_strerror (errno));
		return FALSE;
	}

	fd = socket (PF_INET, SOCK_DGRAM | SOCK_CLOEXEC, 0);
	if (fd < 0) {
		byx_log_trace (LOGD_PLATFORM, "mii[%d,%s]: carrier-detect no: couldn't open control socket: %s", ifindex, ifname, g_strerror (errno));
		return FALSE;
	}
	...
	if (ioctl (fd, SIOCGMIIPHY, &ifr) < 0) {
		byx_log_trace (LOGD_PLATFORM, "mii[%d,%s]: carrier-detect no: SIOCGMIIPHY failed: %s", ifindex, ifname, strerror (errno));
		return FALSE;
	}
	...
	if (ioctl (fd, SIOCGMIIREG, &ifr) != 0) {
		byx_log_trace (LOGD_PLATFORM, "mii[%d,%s]: carrier-detect no: SIOCGMIIREG failed: %s", ifindex, ifname, strerror (errno));
		return FALSE;
	}

	byx_log_trace (LOGD_PLATFORM, "mii[%d,%s]: carrier-detect yes: SIOCGMIIREG result 0x%X", ifindex, ifname, mii->val_out);
	return TRUE;
}
                              ...
                              char *out_ifname)
{
	...
	for (try_count = 0; try_count < 10; try_count++, ifname = NULL) {
		...
		nn = byx_utils_fd_read_loop (fd_ifindex, fd_buf, sizeof (fd_buf) - 2, FALSE);
		if (nn <= 0)
			continue;
		fd_buf[nn] = '\0';

		if (ifindex != _byx_utils_ascii_str_to_int64 (fd_buf, 10, 1, G_MAXINT, -1))
			continue;

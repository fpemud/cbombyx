...
#define _LOG_print(__level, __domain, __errsv, self, ...) \
    G_STMT_START { \
        ...
        _byx_log (__level, __domain, __errsv, NULL, NULL, \
...
#define _LOG(level, domain, self, ...) \
    G_STMT_START { \
        ...
        \
        if (byx_logging_enabled (__level, __domain)) { \
...
#define _LOG_err(errsv, level, domain, self, ...) \
    G_STMT_START { \
        ...
        if (byx_logging_enabled (__level, __domain)) { \
             ...
             * message conveniently (and byx_log would get that right too). But we don't want to depend
                                ...
                                char *buf, gsize buf_size)
{
	...
	case WAIT_FOR_NL_RESPONSE_RESULT_UNKNOWN:
		byx_utils_strbuf_append_str (&buf, &buf_size, "unknown");
		break;
	case WAIT_FOR_NL_RESPONSE_RESULT_RESPONSE_OK:
		byx_utils_strbuf_append_str (&buf, &buf_size, "success");
		break;
	case WAIT_FOR_NL_RESPONSE_RESULT_RESPONSE_UNKNOWN:
		byx_utils_strbuf_append_str (&buf, &buf_size, "failure");
		break;
	default:
		if (seq_result < 0) {
			byx_utils_strbuf_append (&buf, &buf_size, "failure %d (%s%s%s)",
			                        -((int) seq_result),
			...
			byx_utils_strbuf_append (&buf, &buf_size, "internal failure %d", (int) seq_result);
		break;
	}
	return buf0;
}
...
_addrtime_timestamp_to_nm (guint32 timestamp, gint32 *out_now_nm)
{
	...
	g_assert (err == 0);
	now_nm = byx_utils_get_monotonic_timestamp_ms ();
	...
	/* converting the timestamp into byx_utils_get_monotonic_timestamp_ms() scale is
	 ...
	 *   byx_utils_get_monotonic_timestamp_ms() scale.
	 */
	if (result <= 1000)
		return 1;
...
_linktype_read_devtype (int dirfd)
{
	...
	if (byx_utils_file_get_contents (dirfd, "uevent", 1*1024*1024, &contents, NULL, NULL) < 0)
		return NULL;
                 ...
                 gboolean *out_addr_gen_mode_valid)
{
	...
	if (token_valid) {
		*out_token_valid = token_valid;
		byx_utils_ipv6_interface_identifier_get_from_addr (out_token, &i6_token);
	}
                     ...
                     const char *label)
{
	...
	if (   family == AF_INET
		...
		broadcast = *((in_addr_t *) address) | ~_byx_utils_ip4_prefix_to_netmask (plen);
...
#define NM_LINUX_PLATFORM_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMLinuxPlatform, NM_IS_LINUX_PLATFORM, NMPlatform)
...
_log_dbg_sysctl_set_impl (NMPlatform *platform, const char *pathid, int dirfd, const char *path, const char *value)
{
	...
	if (byx_utils_file_get_contents (dirfd, path, 1*1024*1024, &contents, NULL, &error) < 0) {
...
sysctl_set (NMPlatform *platform, const char *pathid, int dirfd, const char *path, const char *value)
{
	...
	if (nwrote == -1) {
		ByxLogLevel level = LOGL_ERR;

		if (errsv == EEXIST) {
			level = LOGL_DEBUG;
		} else if (   errsv == EINVAL
		           && byx_utils_sysctl_ip_conf_is_path (AF_INET6, path, NULL, "mtu")) {
...
static void
_byx_logging_clear_platform_logging_cache_impl (void)
{
...
_log_dbg_sysctl_get_impl (NMPlatform *platform, const char *pathid, const char *contents)
{
	...
	if (!priv->sysctl_get_prev_values) {
		_byx_logging_clear_platform_logging_cache = _byx_logging_clear_platform_logging_cache_impl;
...
delayed_action_to_string_full (DelayedActionType action_type, gpointer user_data, char *buf, gsize buf_size)
{
	...
	byx_utils_strbuf_append_str (&buf, &buf_size, delayed_action_to_string (action_type));
	switch (action_type) {
	case DELAYED_ACTION_TYPE_MASTER_CONNECTED:
		byx_utils_strbuf_append (&buf, &buf_size, " (master-ifindex %d)", GPOINTER_TO_INT (user_data));
		break;
	case DELAYED_ACTION_TYPE_REFRESH_LINK:
		byx_utils_strbuf_append (&buf, &buf_size, " (ifindex %d)", GPOINTER_TO_INT (user_data));
		break;
	case DELAYED_ACTION_TYPE_WAIT_FOR_NL_RESPONSE:
		data = user_data;

		if (data) {
			gint64 timeout = data->timeout_abs_ns - byx_utils_get_monotonic_timestamp_ns ();
			char b[255];

			byx_utils_strbuf_append (&buf, &buf_size, " (seq %u, timeout in %s%"G_GINT64_FORMAT".%09"G_GINT64_FORMAT", response-type %d%s%s)",
			                        data->seq_number,
			                        timeout < 0 ? "-" : "",
			...
			byx_utils_strbuf_append_str (&buf, &buf_size, " (any)");
		break;
	default:
		nm_assert (!user_data);
		break;
...
delayed_action_refresh_all_in_progress (NMPlatform *platform, DelayedActionType action_type)
{
	...
	nm_assert (byx_utils_is_power_of_two (action_type));
	...
	for (i = 0; i < priv->delayed_action.list_wait_for_nl_response->len; ) {
		...
		if (data->seq_result)
			delayed_action_wait_for_nl_response_complete (platform, i, data->seq_result);
		         ...
		         && ((now_ns ?: (now_ns = byx_utils_get_monotonic_timestamp_ns ())) >= data->timeout_abs_ns)) {
...
delayed_action_handle_one (NMPlatform *platform)
{
	...
	if (NM_FLAGS_HAS (priv->delayed_action.flags, DELAYED_ACTION_TYPE_MASTER_CONNECTED)) {
		...
		nm_assert (_byx_utils_ptrarray_find_first ((gconstpointer *) priv->delayed_action.list_master_connected->pdata, priv->delayed_action.list_master_connected->len, user_data) < 0);
	...
	if (NM_FLAGS_HAS (priv->delayed_action.flags, DELAYED_ACTION_TYPE_REFRESH_LINK)) {
		...
		nm_assert (_byx_utils_ptrarray_find_first ((gconstpointer *) priv->delayed_action.list_refresh_link->pdata, priv->delayed_action.list_refresh_link->len, user_data) < 0);
...
delayed_action_schedule (NMPlatform *platform, DelayedActionType action_type, gpointer user_data)
{
	...
	switch (action_type) {
	case DELAYED_ACTION_TYPE_REFRESH_LINK:
		if (_byx_utils_ptrarray_find_first ((gconstpointer *) priv->delayed_action.list_refresh_link->pdata, priv->delayed_action.list_refresh_link->len, user_data) < 0)
			g_ptr_array_add (priv->delayed_action.list_refresh_link, user_data);
		break;
	case DELAYED_ACTION_TYPE_MASTER_CONNECTED:
		if (_byx_utils_ptrarray_find_first ((gconstpointer *) priv->delayed_action.list_master_connected->pdata, priv->delayed_action.list_master_connected->len, user_data) < 0)
                                              ...
                                              gpointer response_out_data)
{
	DelayedActionWaitForNlResponseData data = {
		...
		.timeout_abs_ns = byx_utils_get_monotonic_timestamp_ns () + (200 * (NM_UTILS_NS_PER_SECOND / 1000)),
                 ...
                 const NMPObject *obj_new)
{
	...
	char str_buf[sizeof (_byx_utils_to_string_buffer)];
	char str_buf2[sizeof (_byx_utils_to_string_buffer)];
	NMPCache *cache = nm_platform_get_cache (platform);
                ...
                const ChangeLinkData *data)
{
	...
	ByxLogLevel log_level = LOGL_DEBUG;
          ...
          const NMPlatformLink **out_link)
{
	...
	if (type == NM_LINK_TYPE_BOND) {
		...
		if (!g_file_test ("/sys/class/net/bonding_masters", G_FILE_TEST_EXISTS))
			(void) byx_utils_modprobe (NULL, TRUE, "bonding", "max_bonds=0", NULL);
	}
...
link_set_token (NMPlatform *platform, int ifindex, NMUtilsIPv6IfaceId iid)
{
	...
	_LOGD ("link: change %d: token: set IPv6 address generation token to %s",
	       ifindex, byx_utils_inet6_interface_identifier_to_token (iid, NULL));
...
link_set_address (NMPlatform *platform, int ifindex, gconstpointer address, size_t length)
{
	...
	_LOGD ("link: change %d: address: %s (%lu bytes)", ifindex,
	       (mac = byx_utils_hwaddr_ntoa (address, length)),
              ...
              const NMPlatformLink **out_link)
{
	...
	_LOGD (LOG_FMT_IP_TUNNEL,
	       ...
	       byx_utils_inet4_ntop (props->local, NULL),
	       byx_utils_inet4_ntop (props->remote, buffer));
                 ...
                 const NMPlatformLink **out_link)
{
	...
	_LOGD (LOG_FMT_IP_TUNNEL,
	       ...
	       name,
	       props->parent_ifindex,
	       byx_utils_inet6_ntop (&props->local, NULL),
	       byx_utils_inet6_ntop (&props->remote, buffer));
              ...
              int *out_fd)
{
	...
	byx_utils_ifname_cpy (ifr.ifr_name, name);
                              ...
                              const NMPlatformLink **out_link)
{
	...
	byx_utils_new_infiniband_name (name, ifname_parent, p_key);
	do_request_link (platform, 0, name);
                   ...
                   struct udev_device *udevice)
{
	...
	ifindex = _byx_utils_ascii_str_to_int64 (ifindex_s, 10, 1, G_MAXINT, 0);
	if (ifindex <= 0) {

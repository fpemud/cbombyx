...
#define NM_FAKE_PLATFORM_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMFakePlatform, NM_IS_FAKE_PLATFORM)
...
#define _LOG(level, domain, self, ...) \
    G_STMT_START { \
        ...
        if (byx_logging_enabled (__level, __domain)) { \
            ...
            _byx_log (__level, __domain, 0, NULL, NULL, \
...
infiniband_partition_add (NMPlatform *platform, int parent, int p_key, const NMPlatformLink **out_link)
{
	...
	byx_utils_new_infiniband_name (name, parent_device->obj->link.name, p_key);
                 ...
                 const char *label)
{
	...
	address.plen = plen;
	address.timestamp = byx_utils_get_monotonic_timestamp_s ();
	...
	nmp_cache_iter_for_each (&iter,
		...
		if (addr_family == AF_INET) {
			...
			if (   address->ifindex != ifindex
			    ...
			    || (   peer_addr
			        && (((peer_addr_i ^ address->peer_address) & _byx_utils_ip4_prefix_to_netmask (address->plen)) != 0)))
				continue;
		} else {
	...
	if (has_gateway) {
		...
		nmp_cache_iter_for_each (&iter,
			...
			if (addr_family == AF_INET) {
				...
				guint32 n = byx_utils_ip4_address_clear_host_address (item->network, item->plen);
				guint32 g = byx_utils_ip4_address_clear_host_address (r4->gateway, item->plen);
				...
				if (   r->ifindex == item->ifindex
				    && byx_utils_ip6_address_same_prefix (&r6->gateway, &item->network, item->plen)) {
					has_route_to_gw = TRUE;
					break;
				}
			}
		...
		if (!has_route_to_gw) {
			if (addr_family == AF_INET) {
				byx_log_warn (LOGD_PLATFORM, "Fake platform: failure adding ip4-route '%d: %s/%d %d': Network Unreachable",
				             r->ifindex, byx_utils_inet4_ntop (r4->network, NULL), r->plen, r->metric);
			} else {
				byx_log_warn (LOGD_PLATFORM, "Fake platform: failure adding ip6-route '%d: %s/%d %d': Network Unreachable",
				             r->ifindex, byx_utils_inet6_ntop (&r6->network, NULL), r->plen, r->metric);
			}

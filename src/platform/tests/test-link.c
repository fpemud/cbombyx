...
test_bridge_addr (void)
{
	...
	byx_utils_hwaddr_aton ("de:ad:be:ef:00:11", addr, sizeof (addr));
...
test_software_detect (gconstpointer user_data)
{
	...
	case NM_LINK_TYPE_GRE: {
		...
		if (!nm_platform_link_get_by_ifname (NM_PLATFORM_GET, "gre0")) {
			...
			gracefully_skip = byx_utils_modprobe (NULL, TRUE, "ip_gre", NULL) != 0;
		}
	...
	case NM_LINK_TYPE_IPIP: {
		...
		if (!nm_platform_link_get_by_ifname (NM_PLATFORM_GET, "tunl0")) {
			...
			gracefully_skip = byx_utils_modprobe (NULL, TRUE, "ipip", NULL) != 0;
		}
	...
	case NM_LINK_TYPE_IP6TNL: {
		...
		if (!nm_platform_link_get_by_ifname (NM_PLATFORM_GET, "ip6tnl0")) {
			...
			gracefully_skip = byx_utils_modprobe (NULL, TRUE, "ip6_tunnel", NULL) != 0;
		}
	...
	case NM_LINK_TYPE_SIT: {
		...
		if (!nm_platform_link_get_by_ifname (NM_PLATFORM_GET, "sit0")) {
			...
			gracefully_skip = byx_utils_modprobe (NULL, TRUE, "sit", NULL) != 0;
		}
...
{
	gint64 time, start_time = byx_utils_get_monotonic_timestamp_ns ();
	guint i;
	char name[64];
	const NMPlatformLink *pllink;
	...
	time = byx_utils_get_monotonic_timestamp_ns () - start_time;
...
test_sysctl_rename (void)
{
	...
	case 0: {
		...
		if (byx_utils_file_get_contents (dirfd, "ifindex", 1*1024*1024, &c, NULL, NULL) < 0)
			g_assert_not_reached();
		g_assert_cmpint (ifindex[0], ==, (int) _byx_utils_ascii_str_to_int64 (c, 10, 0, G_MAXINT, -1));
		break;
	}
	case 1: {
...
test_sysctl_netns_switch (void)
{
	 ...
	 * either using dirfd or via the platform instance (which switches namespace as needed). */
	{
		...
		if (byx_utils_file_get_contents (dirfd, "ifindex", 0, &c, NULL, NULL) < 0)
			g_assert_not_reached();
		g_assert_cmpint (ifindex, ==, (int) _byx_utils_ascii_str_to_int64 (c, 10, 0, G_MAXINT, -1));
	}
	 ...
	 * in which we created the link. */
	{
		...
		if (byx_utils_file_get_contents (-1, nm_sprintf_bufa (100, "/sys/class/net/%s/ifindex", IFNAME), 0, &c, NULL, NULL) < 0)
			ifindex_tmp = -1;
		else
			ifindex_tmp = _byx_utils_ascii_str_to_int64 (c, 10, 0, G_MAXINT, -2);
	}

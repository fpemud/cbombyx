                                  ...
                                  guint8 plen)
{
	...
	addr = byx_utils_ip4_address_clear_host_address (addr, plen);
	NMTST_WAIT_ASSERT (400, {
		...
		nmp_cache_iter_for_each (&iter,
			...
			if (   r->plen == plen
			    && addr == byx_utils_ip4_address_clear_host_address (r->network, plen)
			    && r->metric == 0
...
test_ip6_route (void)
{
	...
	rts[0].metric = byx_utils_ip6_route_metric_normalize (metric);
	rts[0].mss = mss;
	rts[1].rt_source = nmp_utils_ip_config_source_round_trip_rtprot (NM_IP_CONFIG_SOURCE_USER);
	rts[1].network = network;
	rts[1].plen = plen;
	...
	rts[1].metric = byx_utils_ip6_route_metric_normalize (metric);
	rts[1].mss = mss;
	rts[2].rt_source = nmp_utils_ip_config_source_round_trip_rtprot (NM_IP_CONFIG_SOURCE_USER);
	...
	rts[2].metric = byx_utils_ip6_route_metric_normalize (metric);
	rts[2].mss = mss;
	g_assert_cmpint (routes->len, ==, 3);

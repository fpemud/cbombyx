                                 ...
                                 guint8 tos)
{
	...
	if (c != c_exists && c_exists != -1) {
		g_error ("[%s:%u] %s(): The ip4 route %s/%d metric %u tos %u shall exist %u times, but platform has it %u times",
		         file, line, func,
		         byx_utils_inet4_ntop (network, NULL), plen,
		         metric,
		         tos,
		         c_exists,
		         c);
                                 ...
                                 guint8 src_plen)
{
	...
	if (c != c_exists && c_exists != -1) {
		...
		g_error ("[%s:%u] %s(): The ip6 route %s/%d metric %u src %s/%d shall exist %u times, but platform has it %u times",
		         file, line, func,
		         byx_utils_inet6_ntop (network, s_network),
		         plen,
		         metric,
		         byx_utils_inet6_ntop (src, s_src),
		         src_plen,
		         c_exists,
		         c);
	}
...
nmtstp_wait_for_signal_until (NMPlatform *platform, gint64 until_ms)
{
	...
	while (TRUE) {
		now = byx_utils_get_monotonic_timestamp_ms ();
...
nmtstp_wait_for_link (NMPlatform *platform, const char *ifname, NMLinkType expected_link_type, gint64 timeout_ms)
{
	return nmtstp_wait_for_link_until (platform, ifname, expected_link_type,
	                                   timeout_ms
	                                     ? byx_utils_get_monotonic_timestamp_ms () + timeout_ms
	                                     : 0);
}
                 ...
                 const char *label)
{
	...
	if (external_command) {
		...
		if (is_v4) {
			...
			if (   peer_address->addr4 != address->addr4
				...
				g_snprintf (s_peer, sizeof (s_peer), " peer %s", byx_utils_inet4_ntop (peer_address->addr4, b2));
			} else
				s_peer[0] = '\0';

			nmtstp_run_command_check ("ip address change %s%s/%d dev %s%s%s%s",
			                          byx_utils_inet4_ntop (address->addr4, b1),
			                          s_peer,
			                          plen,
			                          ifname,
			                          s_valid ?: "",
			...
			nmtstp_run_command_check ("ip address change %s%s%s/%d dev %s%s%s%s",
			                          byx_utils_inet6_ntop (&address->addr6, b1),
			                          !IN6_IS_ADDR_UNSPECIFIED (&peer_address->addr6) ? " peer " : "",
			                          !IN6_IS_ADDR_UNSPECIFIED (&peer_address->addr6) ? byx_utils_inet6_ntop (&peer_address->addr6, b2) : "",
			                          plen,
			                          ifname,
			                          s_valid ?: "",
			                          s_preferred ?: "",
	...
	end_time = byx_utils_get_monotonic_timestamp_ms () + 250;
	do {
	...
	if (external_command) {
		...
		if (is_v4) {
			success = nmtstp_run_command ("ip address delete %s%s%s/%d dev %s",
			                              ...
			                              peer_address->addr4 != address->addr4 ? byx_utils_inet4_ntop (peer_address->addr4, b2) : "",
			                              plen,
			                              ifname);
		} else {
                      ...
                      const char *peer)
{
	...
	g_assert (byx_utils_is_valid_iface_name (name, NULL));
                     ...
                     const NMPlatformLnkGre *lnk)
{
	...
	if (external_command) {
		...
		success = !nmtstp_run_command ("ip tunnel add %s mode gre %s local %s remote %s ttl %u tos %02x %s",
		                                ...
		                                byx_utils_inet4_ntop (lnk->remote, buffer),
		                                lnk->ttl,
		                                lnk->tos,
                        ...
                        const NMPlatformLnkIp6Tnl *lnk)
{
	...
	if (external_command) {
		...
		success = !nmtstp_run_command ("ip -6 tunnel add %s mode %s %s local %s remote %s ttl %u tclass %s encaplimit %s flowlabel %x",
		                                ...
		                                mode,
		                                dev,
		                                byx_utils_inet6_ntop (&lnk->local, NULL),
		                                byx_utils_inet6_ntop (&lnk->remote, buffer),
		                                lnk->ttl,
                       ...
                       const NMPlatformLnkVxlan *lnk)
{
	...
	if (external_command) {
		...
		if (lnk->local)
			local = g_strdup_printf ("%s", byx_utils_inet4_ntop (lnk->local, NULL));
		else if (memcmp (&lnk->local6, &in6addr_any, sizeof (in6addr_any)))
			local = g_strdup_printf ("%s", byx_utils_inet6_ntop (&lnk->local6, NULL));

		if (lnk->group)
			remote = g_strdup_printf ("%s", byx_utils_inet4_ntop (lnk->group, NULL));
		else if (memcmp (&lnk->group6, &in6addr_any, sizeof (in6addr_any)))
			remote = g_strdup_printf ("%s", byx_utils_inet6_ntop (&lnk->group6, NULL));
                       ...
                       NMLinkType link_type)
{
	...
	g_assert (!name || byx_utils_is_valid_iface_name (name, NULL));

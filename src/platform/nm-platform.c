                              ...
                              gsize n_egress_map)
{
	...
	if (_LOGD_ENABLED ()) {
		...
		if (flags_mask)
			byx_utils_strbuf_append (&b, &len, " flags 0x%x/0x%x", (unsigned) flags_set, (unsigned) flags_mask);

		if (ingress_reset_all || n_ingress_map) {
			byx_utils_strbuf_append_str (&b, &len, " ingress-qos-map");
			nm_platform_vlan_qos_mapping_to_string ("", ingress_map, n_ingress_map, b, len);
			i = strlen (b);
			b += i;
			len -= i;
			if (ingress_reset_all)
				byx_utils_strbuf_append_str (&b, &len, " (reset-all)");
		}

		if (egress_reset_all || n_egress_map) {
			byx_utils_strbuf_append_str (&b, &len, " egress-qos-map");
                               ...
                               const NMPlatformLink **out_link)
{
	...
	byx_utils_new_infiniband_name (name, parent_link->name, p_key);
...
nm_platform_ip6_address_delete (NMPlatform *self, int ifindex, struct in6_addr address, guint8 plen)
{
	...
	_LOGD ("address: deleting IPv6 address %s/%d, ifindex %d%s",
	       byx_utils_inet6_ntop (&address, NULL), plen, ifindex,
	       _to_string_dev (self, ifindex, str_dev, sizeof (str_dev)));
                              ...
                              GPtrArray *known_addresses)
{
	...
	for (i = 0; i < known_addresses->len; i++) {
		...
		lifetime = byx_utils_lifetime_get (known_address->timestamp, known_address->lifetime, known_address->preferred,
		                                  now, &preferred);
		if (!lifetime)

...
static int _byx_utils_testing = 0;

gboolean
byx_utils_get_testing_initialized ()
{
	NMUtilsTestFlags flags;

	flags = (NMUtilsTestFlags) _byx_utils_testing;
	if (flags == NM_UTILS_TEST_NONE)
		flags = (NMUtilsTestFlags) g_atomic_int_get (&_byx_utils_testing);
	return flags != NM_UTILS_TEST_NONE;
}

NMUtilsTestFlags
byx_utils_get_testing ()
{
	NMUtilsTestFlags flags;
	...
	/* Accessing byx_utils_get_testing() causes us to set the flags to initialized.
	 * Detecting running tests also based on g_test_initialized(). */
	flags = _NM_UTILS_TEST_INITIALIZED;
	if (g_test_initialized ())
		flags |= _NM_UTILS_TEST_GENERAL;

	if (g_atomic_int_compare_and_exchange (&_byx_utils_testing, 0, (int) flags)) {
		/* Done. We set it. */
		return flags & NM_UTILS_TEST_ALL;
	}
	/* It changed in the meantime (??). Re-read the value. */
	return ((NMUtilsTestFlags) _byx_utils_testing) & NM_UTILS_TEST_ALL;
}

void
_byx_utils_set_testing (NMUtilsTestFlags flags)
{
	...
	if (!g_atomic_int_compare_and_exchange (&_byx_utils_testing, 0, (int) flags)) {
		/* We only allow setting _byx_utils_set_testing() once, before fetching the
		 * value with byx_utils_get_testing(). */
		g_return_if_reached ();
	}
}
...
_nm_singleton_instance_destroy (void)
{
	...
	while (_singletons) {
		...
		if (instance->ref_count > 1)
			byx_log_dbg (LOGD_CORE, "disown %s singleton (%p)", G_OBJECT_TYPE_NAME (instance), instance);
...
void
_byx_singleton_instance_register_destruction (GObject *instance)
{
...
gconstpointer
byx_utils_ipx_address_clear_host_address (int family, gpointer dst, gconstpointer src, guint8 plen)
{
	...
	case AF_INET:
		...
		*((guint32 *) dst) = byx_utils_ip4_address_clear_host_address (*((guint32 *) src), plen);
		break;
	case AF_INET6:
		g_return_val_if_fail (plen <= 128, NULL);
		byx_utils_ip6_address_clear_host_address (dst, src, plen);
		break;
	default:
...
/* byx_utils_ip4_address_clear_host_address:
 ...
 */
in_addr_t
byx_utils_ip4_address_clear_host_address (in_addr_t addr, guint8 plen)
{
	return addr & _byx_utils_ip4_prefix_to_netmask (plen);
}

/* byx_utils_ip6_address_clear_host_address:
...
byx_utils_ip6_address_clear_host_address (struct in6_addr *dst, const struct in6_addr *src, guint8 plen)
{
	g_return_val_if_fail (plen <= 128, NULL);
	g_return_val_if_fail (dst, NULL);
...
int
byx_utils_ip6_address_same_prefix_cmp (const struct in6_addr *addr_a, const struct in6_addr *addr_b, guint8 plen)
{
	int nbytes;
	guint8 va, vb, m;
...
void
byx_utils_array_remove_at_indexes (GArray *array, const guint *indexes_to_delete, gsize len)
{
	gsize elt_size;
...
int
byx_utils_modprobe (GError **error, gboolean suppress_error_logging, const char *arg1, ...)
{
...
#define ARGV_TO_STR(argv)   (_log_str ? _log_str : (_log_str = g_strjoinv (" ", (char **) argv->pdata)))
	GError *local = NULL;
	va_list ap;
	ByxLogLevel llevel = suppress_error_logging ? LOGL_DEBUG : LOGL_ERR;
	...
	byx_log_dbg (LOGD_CORE, "modprobe: '%s'", ARGV_TO_STR (argv));
	if (!g_spawn_sync (NULL, (char **) argv->pdata, NULL, 0, NULL, NULL, &std_out, &std_err, &exit_status, &local)) {
		byx_log (llevel, LOGD_CORE, NULL, NULL, "modprobe: '%s' failed: %s", ARGV_TO_STR (argv), local->message);
		g_propagate_error (error, local);
		return -1;
	} else if (exit_status != 0) {
		byx_log (llevel, LOGD_CORE, NULL, NULL, "modprobe: '%s' exited with error %d%s%s%s%s%s%s", ARGV_TO_STR (argv), exit_status,
...
{
	g_snprintf (buf, KC_WAITED_TO_STRING, " (%ld usec elapsed)", (long) (byx_utils_get_monotonic_timestamp_us () - wait_start_us));
	return buf;
}
...
_kc_cb_watch_child (GPid pid, gint status, gpointer user_data)
{
	...
	byx_log_dbg (data->log_domain, "%s: terminated %s%s",
...
_kc_cb_timeout_grace_period (void *user_data)
{
	...
	if ((ret = kill (data->pid, SIGKILL)) != 0) {
		...
		if (errsv != ESRCH) {
			byx_log_err (LOGD_CORE | data->log_domain, "%s: kill(SIGKILL) returned unexpected return value %d: (%s, %d)",
			            data->log_name, ret, strerror (errsv), errsv);
		}
	} else {
		byx_log_dbg (data->log_domain, "%s: process not terminated after %ld usec. Sending SIGKILL signal",
		            data->log_name, (long) (byx_utils_get_monotonic_timestamp_us () - data->async.wait_start_us));
	}
...
_kc_invoke_callback_idle (gpointer user_data)
{
	...
	if (data->sync.success) {
		...
		byx_log_dbg (data->log_domain, "%s: invoke callback: terminated %s",
		            data->log_name, _kc_exit_to_string (buf_exit, data->sync.child_status));
	} else
		byx_log_dbg (data->log_domain, "%s: invoke callback: killing child failed", data->log_name);
 ...
 **/
void
byx_utils_kill_child_async (pid_t pid, int sig, NMLogDomain log_domain,
                           ...
                           NMUtilsKillChildAsyncCb callback, void *user_data)
{
	...
	if (ret > 0) {
		byx_log_dbg (log_domain, LOG_NAME_FMT ": process %ld already terminated %s",
		...
		if (errsv != ECHILD) {
			byx_log_err (LOGD_CORE | log_domain, LOG_NAME_FMT ": unexpected error while waitpid: %s (%d)",
	...
	if (kill (pid, sig) != 0) {
		...
		/* ESRCH means, process does not exist or is already a zombie. */
		if (errsv != ESRCH) {
			byx_log_err (LOGD_CORE | log_domain, LOG_NAME_FMT ": unexpected error sending %s: %s (%d)",
		...
		if (ret > 0) {
			...
			errsv = errno;
			byx_log_err (LOGD_CORE | log_domain, LOG_NAME_FMT ": failed due to unexpected return value %ld by waitpid (%s, %d) after sending %s",
	...
	data = _kc_async_data_alloc (pid, log_domain, log_name, callback, user_data);
	data->async.wait_start_us = byx_utils_get_monotonic_timestamp_us ();
	...
	if (sig != SIGKILL && wait_before_kill_msec > 0) {
		...
		byx_log_dbg (log_domain, "%s: wait for process to terminate after sending %s (send SIGKILL in %ld milliseconds)...",
		            data->log_name,  _kc_signal_to_string (sig), (long) wait_before_kill_msec);
	} else {
		data->async.source_timeout_kill_id = 0;
		byx_log_dbg (log_domain, "%s: wait for process to terminate after sending %s...",
 ...
 **/
gboolean
byx_utils_kill_child_sync (pid_t pid, int sig, NMLogDomain log_domain, const char *log_name,
                          ...
                          guint32 sleep_duration_msec)
{
	...
	if (kill (pid, sig) != 0) {
		...
		/* ESRCH means, process does not exist or is already a zombie. */
		if (errsv != ESRCH) {
			byx_log_err (LOGD_CORE | log_domain, LOG_NAME_FMT ": failed to send %s: %s (%d)",
			            LOG_NAME_ARGS, _kc_signal_to_string (sig), strerror (errsv), errsv);
		} else {
			/* let's try again with waitpid, probably there was a race... */
			ret = waitpid (pid, &status, 0);
	...
	wait_start_us = byx_utils_get_monotonic_timestamp_us ();
	...
	if (sig != SIGKILL) {
		...
		while (TRUE) {
			ret = waitpid (pid, &status, WNOHANG);
			if (ret > 0) {
				byx_log_dbg (log_domain, LOG_NAME_FMT ": after sending %s, process %ld exited %s%s",
			...
			if (ret == -1) {
				...
				/* ECHILD means, the process is not a child/does not exist or it has SIGCHILD blocked. */
				if (errsv != ECHILD) {
					byx_log_err (LOGD_CORE | log_domain, LOG_NAME_FMT ": after sending %s, waitpid failed with %s (%d)%s",
					            LOG_NAME_ARGS, _kc_signal_to_string (sig), strerror (errsv), errsv,
					           was_waiting ? _kc_waited_to_string (buf_wait, wait_start_us) : "");
					goto out;
				}
			...
			if (!was_waiting) {
				byx_log_dbg (log_domain, LOG_NAME_FMT ": waiting up to %ld milliseconds for process to terminate normally after sending %s...",
		...
		if (wait_until) {
			byx_log_dbg (log_domain, LOG_NAME_FMT ": sending SIGKILL...", LOG_NAME_ARGS);
			...
			if (kill (pid, SIGKILL) != 0) {
				...
				/* ESRCH means, process does not exist or is already a zombie. */
				if (errsv != ESRCH) {
					byx_log_err (LOGD_CORE | log_domain, LOG_NAME_FMT ": failed to send SIGKILL (after sending %s), %s (%d)",
								LOG_NAME_ARGS, _kc_signal_to_string (sig), strerror (errsv), errsv);
					goto out;
				}
			}
	...
	if (!was_waiting) {
		byx_log_dbg (log_domain, LOG_NAME_FMT ": waiting for process to terminate after sending %s%s...",
	...
	while ((ret = waitpid (pid, &status, 0)) <= 0) {
		...
		if (errsv != EINTR) {
			byx_log_err (LOGD_CORE | log_domain, LOG_NAME_FMT ": after sending %s%s, waitpid failed with %s (%d)%s",
	...
	byx_log_dbg (log_domain, LOG_NAME_FMT ": after sending %s%s, process %ld exited %s%s",
...
/* byx_utils_kill_process_sync:
 * @pid: process id to kill
 * @start_time: the start time of the process to kill (as obtained by byx_utils_get_start_time_for_pid()).
 ...
 **/
void
byx_utils_kill_process_sync (pid_t pid, guint64 start_time, int sig, NMLogDomain log_domain,
                            const char *log_name, guint32 wait_before_kill_msec,
                            guint32 sleep_duration_msec, guint32 max_wait_msec)
{
	...
	start_time0 = byx_utils_get_start_time_for_pid (pid, &p_state, NULL);
	if (start_time0 == 0) {
		byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": cannot kill process %ld because it seems already gone",
		            LOG_NAME_ARGS, (long int) pid);
		return;
	}
	if (start_time != 0 && start_time != start_time0) {
		byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": don't kill process %ld because the start_time is unexpectedly %lu instead of %ld",
	...
	case 'x':
	case 'X':
		byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": cannot kill process %ld because it is already a zombie (%c)",
	...
	if (kill (pid, sig) != 0) {
		...
		/* ESRCH means, process does not exist or is already a zombie. */
		if (errsv == ESRCH) {
			byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": failed to send %s because process seems gone",
			            LOG_NAME_ARGS, _kc_signal_to_string (sig));
		} else {
			byx_log_warn (LOGD_CORE | log_domain, LOG_NAME_PROCESS_FMT ": failed to send %s: %s (%d)",
			             LOG_NAME_ARGS, _kc_signal_to_string (sig), strerror (errsv), errsv);
		}
		return;
	}
	...
	while (TRUE) {
		start_time = byx_utils_get_start_time_for_pid (pid, &p_state, NULL);

		if (start_time != start_time0) {
			byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": process is gone after sending signal %s%s",
			            LOG_NAME_ARGS, _kc_signal_to_string (sig),
			            was_waiting ? _kc_waited_to_string (buf_wait, wait_start_us) : "");
			return;
		}
		...
		case 'x':
		case 'X':
			byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": process is a zombie (%c) after sending signal %s%s",
		...
		if (kill (pid, 0) != 0) {
			...
			/* ESRCH means, process does not exist or is already a zombie. */
			if (errsv == ESRCH) {
				byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": process is gone or a zombie after sending signal %s%s",
				            LOG_NAME_ARGS, _kc_signal_to_string (sig),
				            was_waiting ? _kc_waited_to_string (buf_wait, wait_start_us) : "");
			} else {
				byx_log_warn (LOGD_CORE | log_domain, LOG_NAME_PROCESS_FMT ": failed to kill(%ld, 0): %s (%d)%s",
		...
		if (   max_wait_until != 0
			...
			if (wait_until_sigkill != 0) {
				...
				byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": sending SIGKILL", LOG_NAME_ARGS);
				kill (pid, SIGKILL);
			}
			byx_log_warn (log_domain, LOG_NAME_PROCESS_FMT ": timeout %u msec waiting for process to disappear (after sending %s)%s",
		...
		if (wait_until_sigkill != 0) {
			if (now >= wait_until_sigkill) {
				...
				if (kill (pid, SIGKILL) != 0) {
					...
					/* ESRCH means, process does not exist or is already a zombie. */
					if (errsv != ESRCH) {
						byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": process is gone or a zombie%s",
						            LOG_NAME_ARGS, _kc_waited_to_string (buf_wait, wait_start_us));
					} else {
						byx_log_warn (LOGD_CORE | log_domain, LOG_NAME_PROCESS_FMT ": failed to send SIGKILL (after sending %s), %s (%d)%s",
						             LOG_NAME_ARGS, _kc_signal_to_string (sig), strerror (errsv), errsv,
						             _kc_waited_to_string (buf_wait, wait_start_us));
					}
					return;
		...
		if (!was_waiting) {
			if (wait_until_sigkill != 0) {
				byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": waiting up to %ld milliseconds for process to disappear before sending KILL signal after sending %s...",
				            LOG_NAME_ARGS, (long) wait_before_kill_msec, _kc_signal_to_string (sig));
			} else if (max_wait_until != 0) {
				byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": waiting up to %ld milliseconds for process to disappear after sending %s...",
				            LOG_NAME_ARGS, (long) max_wait_msec, _kc_signal_to_string (sig));
			} else {
				byx_log_dbg (log_domain, LOG_NAME_PROCESS_FMT ": waiting for process to disappear after sending %s...",
				            LOG_NAME_ARGS, _kc_signal_to_string (sig));
			}
			was_waiting = TRUE;
		}
...
const char *
byx_utils_find_helper(const char *progname, const char *try_first, GError **error)
{
	return byx_utils_file_search_in_paths (progname, try_first, NM_PATHS_DEFAULT, G_FILE_TEST_IS_EXECUTABLE, NULL, NULL, error);
}

/*****************************************************************************/

/**
 * byx_utils_read_link_absolute:
 ...
 **/
char *
byx_utils_read_link_absolute (const char *link_file, GError **error)
{
...
match_device_s390_subchannels_parse (const char *s390_subchannels, guint32 *out_a, guint32 *out_b, guint32 *out_c)
{
	...
	*pc++ = '\0';
	...
	a = _byx_utils_ascii_str_to_int64 (pa, 16, 0, G_MAXUINT32, -1);
	if (a == -1)
		return FALSE;
	b = _byx_utils_ascii_str_to_int64 (pb, 16, 0, G_MAXUINT32, -1);
	if (b == -1)
		return FALSE;
	c = _byx_utils_ascii_str_to_int64 (pc, 16, 0, G_MAXUINT32, -1);
	if (c == -1)
		return FALSE;
                          ...
                          MatchDeviceData *match_data)
{
	if (G_UNLIKELY (!match_data->hwaddr.is_parsed)) {
		...
		if (match_data->hwaddr.value) {
			...
			if (!_byx_utils_hwaddr_aton (match_data->hwaddr.value, match_data->hwaddr.bin, sizeof (match_data->hwaddr.bin), &l))
	...
	return byx_utils_hwaddr_matches (spec_str, -1, match_data->hwaddr.bin, match_data->hwaddr.len);
}
                      ...
                      const char *s390_subchannels)
{
	...
	nm_assert (!hwaddr || byx_utils_hwaddr_valid (hwaddr, -1));
...
match_config_eval (const char *str, const char *tag, guint cur_nm_version)
{
	...
	v_maj = _byx_utils_ascii_str_to_int64 (s_ver_tokens[0], 10, 0, 0xFFFF, -1);
	if (v_maj < 0)
		return FALSE;
	if (n_tokens >= 2) {
		v_min = _byx_utils_ascii_str_to_int64 (s_ver_tokens[1], 10, 0, 0xFF, -1);
		if (v_min < 0)
			return FALSE;
	}
	if (n_tokens >= 3) {
		v_mic = _byx_utils_ascii_str_to_int64 (s_ver_tokens[2], 10, 0, 0xFF, -1);
		if (v_mic < 0)
			return FALSE;
	}
...
char *
byx_utils_new_vlan_name (const char *parent_iface, guint32 vlan_id)
{
	guint id_len;
	gsize parent_len;
	char *ifname;
 ...
 */
const char *
byx_utils_new_infiniband_name (char *name, const char *parent_name, int p_key)
{
	 ...
	 * want to assert against that in byx_utils_new_infiniband_name(). So be more
...
/**
 * byx_utils_cmp_connection_by_autoconnect_priority:
 * @a:
 * @b:
 *
 ...
 */
int
byx_utils_cmp_connection_by_autoconnect_priority (NMConnection *a, NMConnection *b)
{
...
monotonic_timestamp_get (struct timespec *tp)
{
	 ...
	 * value byx_utils_get_monotonic_timestamp_*() might return.
	 * For this to be true also for byx_utils_get_monotonic_timestamp_s() at
	 ...
	 * range of signed int, before the time stamp for byx_utils_get_monotonic_timestamp_s()
	 * wraps (~68 years).
	 **/
	monotonic_timestamp_offset_sec = (- ((gint64) tp->tv_sec)) + 1;
	monotonic_timestamp_clock_mode = clock_mode;

	if (byx_logging_enabled (LOGL_DEBUG, LOGD_CORE)) {
		time_t now = time (NULL);
		struct tm tm;
		char s[255];

		strftime (s, sizeof (s), "%Y-%m-%d %H:%M:%S", localtime_r (&now, &tm));
		byx_log_dbg (LOGD_CORE, "monotonic timestamp started counting 1.%09ld seconds ago with "
 ...
 *
 * All the byx_utils_get_monotonic_timestamp_*s functions return the same
 * timestamp but in different scales (nsec, usec, msec, sec).
 **/
gint64
byx_utils_get_monotonic_timestamp_ns (void)
{
...
/**
 * byx_utils_get_monotonic_timestamp_us:
 *
 ...
 **/
gint64
byx_utils_get_monotonic_timestamp_us (void)
{
	struct timespec tp = { 0 };
...
/**
 * byx_utils_get_monotonic_timestamp_ms:
 *
 ...
 **/
gint64
byx_utils_get_monotonic_timestamp_ms (void)
{
	struct timespec tp = { 0 };
...
/**
 * byx_utils_get_monotonic_timestamp_s:
 *
 * Returns: byx_utils_get_monotonic_timestamp_ms() in seconds (throwing
 ...
 **/
gint32
byx_utils_get_monotonic_timestamp_s (void)
{
	struct timespec tp = { 0 };
...
void
byx_utils_log_connection_diff (NMConnection *connection,
	...
	for (i = 0; i < sorted_hashes->len; i++) {
		...
		for (j = 0; j < sorted_names->len; j++) {
			...
			if (print_header) {
				...
				if (diff_base) {
					t2 = nm_connection_get_connection_type (diff_base);
					byx_log (level, domain, NULL, NULL, "%sconnection '%s' (%p/%s/%s%s%s < %p/%s/%s%s%s)%s%s%s:",
					        prefix, name,
					        connection, G_OBJECT_TYPE_NAME (connection), NM_PRINT_FMT_QUOTE_STRING (t1),
					        diff_base, G_OBJECT_TYPE_NAME (diff_base), NM_PRINT_FMT_QUOTE_STRING (t2),
					        NM_PRINT_FMT_QUOTED (dbus_path, " [", dbus_path, "]", ""));
				} else {
					byx_log (level, domain, NULL, NULL, "%sconnection '%s' (%p/%s/%s%s%s):%s%s%s",
					        prefix, name,
					        connection, G_OBJECT_TYPE_NAME (connection), NM_PRINT_FMT_QUOTE_STRING (t1),
					        NM_PRINT_FMT_QUOTED (dbus_path, " [", dbus_path, "]", ""));
				}
				...
				if (!nm_connection_verify (connection, &err_verify)) {
					byx_log (level, domain, NULL, NULL, "%sconnection %p does not verify: %s", prefix, connection, err_verify->message);
...
#define _NM_LOG_ALIGN "-25"
			if (print_setting_header) {
				if (diff_base) {
					...
					byx_log (level, domain, NULL, NULL, "%s%"_NM_LOG_ALIGN"s [ %s ]", prefix, setting_data->name, str1->str);
				} else
					byx_log (level, domain, NULL, NULL, "%s%"_NM_LOG_ALIGN"s [ %p ]", prefix, setting_data->name, setting_data->setting);
			...
			switch (item->diff_result & (NM_SETTING_DIFF_RESULT_IN_A | NM_SETTING_DIFF_RESULT_IN_B)) {
				case NM_SETTING_DIFF_RESULT_IN_B:
					byx_log (level, domain, NULL, NULL, "%s%"_NM_LOG_ALIGN"s < %s", prefix, str1->str, str_diff ?: "NULL");
					break;
				case NM_SETTING_DIFF_RESULT_IN_A:
					byx_log (level, domain, NULL, NULL, "%s%"_NM_LOG_ALIGN"s = %s", prefix, str1->str, str_conn ?: "NULL");
					break;
				default:
					byx_log (level, domain, NULL, NULL, "%s%"_NM_LOG_ALIGN"s = %s < %s", prefix, str1->str, str_conn ?: "NULL", str_diff ?: "NULL");
					break;
#undef _NM_LOG_ALIGN
			}
			g_free (str_conn);
...
/**
 * byx_utils_monotonic_timestamp_as_boottime:
 ...
 **/
gint64
byx_utils_monotonic_timestamp_as_boottime (gint64 timestamp, gint64 timestamp_ns_per_tick)
{
	gint64 offset;
 ...
 */
const char *
byx_utils_sysctl_ip_conf_path (int addr_family, char *buf, const char *ifname, const char *property)
{
	int len;
	...
	g_assert (byx_utils_is_valid_iface_name (ifname, NULL));
...
gboolean
byx_utils_sysctl_ip_conf_is_path (int addr_family, const char *path, const char *ifname, const char *property)
{
	g_return_val_if_fail (path, FALSE);
	NM_ASSERT_VALID_PATH_COMPONENT (property);
	g_assert (!ifname || byx_utils_is_valid_iface_name (ifname, NULL));
	...
	if (ifname) {
		...
		memcpy (buf, path, l);
		buf[l] = '\0';
		if (!byx_utils_is_valid_iface_name (buf, NULL))
			return FALSE;
		path = slash + 1;
	}
...
gboolean
byx_utils_is_valid_path_component (const char *name)
{
	const char *n;
...
{
	if (G_LIKELY (byx_utils_is_valid_path_component (name)))
		return name;

	byx_log_err (LOGD_CORE, "Failed asserting path component: %s%s%s",
...
gboolean
byx_utils_is_specific_hostname (const char *name)
{
	if (!name)
		return FALSE;
	if (   strcmp (name, "(none)")
...
gboolean
byx_utils_machine_id_parse (const char *id_str, /*uuid_t*/ guchar *out_uuid)
{
	int i;
	guint8 v0, v1;
...
char *
byx_utils_machine_id_read (void)
{
 ...
 */
int
byx_utils_fd_get_contents (int fd,
                          gboolean close_fd,
                          gsize max_length,
                          char **contents,
                          gsize *length,
                          GError **error)
{
	...
	if (   stat_buf.st_size > 0
		...
		n_read = byx_utils_fd_read_loop (fd, str, n_stat, TRUE);
		if (n_read < 0)
 ...
 *   WARNING: see byx_utils_fd_get_contents() hint about @max_length.
 * @contents: the output buffer with the file read. It is always
 *   NUL terminated. The buffer is at most @max_length long, including
 *  the NUL byte. That is, it reads only files up to a length of
 *  @max_length - 1 bytes.
 ...
 */
int
byx_utils_file_get_contents (int dirfd,
                            const char *filename,
                            gsize max_length,
                            char **contents,
                            gsize *length,
                            GError **error)
{
	...
	return byx_utils_fd_get_contents (fd,
	                                 TRUE,
	                                 max_length,
	                                 contents,
	                                 length,
                  ...
                  gsize *out_key_len)
{
	...
	if (g_file_get_contents (NMSTATEDIR "/secret_key", (char **) &secret_key, &key_len, &error)) {
		 ...
		 * a new one. */
		byx_log_warn (LOGD_CORE, "secret-key: too short secret key in \"%s\" (generate new key)", NMSTATEDIR "/secret_key");
		nm_clear_g_free (&secret_key);
	} else {
		if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND)) {
			byx_log_warn (LOGD_CORE, "secret-key: failure reading secret key in \"%s\": %s (generate new key)",
	...
	if (!byx_utils_random_bytes (secret_key, key_len)) {
		byx_log_warn (LOGD_CORE, "secret-key: failure to generate good random data for secret-key (use non-persistent key)");
		success = FALSE;
		goto out;
	}

	if (!byx_utils_file_set_contents (NMSTATEDIR "/secret_key", (char *) secret_key, key_len, 0077, &error)) {
		byx_log_warn (LOGD_CORE, "secret-key: failure to persist secret key in \"%s\" (%s) (use non-persistent key)",
		             NMSTATEDIR "/secret_key", error->message);
		success = FALSE;
		goto out;
	}
...
gboolean
byx_utils_secret_key_get (const guint8 **out_secret_key,
                         gsize *out_key_len)
{
...
gint64
byx_utils_secret_key_get_timestamp (void)
{
	struct stat stat_buf;
	const guint8 *key;
	gsize key_len;

	if (!byx_utils_secret_key_get (&key, &key_len))
		return 0;
...
byx_utils_get_boot_id (void)
{
	...
	if (G_UNLIKELY (!boot_id)) {
		...
		byx_utils_file_get_contents (-1, "/proc/sys/kernel/random/boot_id", 0,
		                            &contents, NULL, NULL);
		if (contents) {
			g_strstrip (contents);
			if (contents[0]) {
				 ...
				 * ends.
				 * byx_utils_file_get_contents() likely allocated a larger
		...
		if (!boot_id)
			boot_id = byx_utils_uuid_generate ();
	}
...
/**
 * byx_utils_get_ipv6_interface_identifier:
 ...
 */
gboolean
byx_utils_get_ipv6_interface_identifier (NMLinkType link_type,
                                        const guint8 *hwaddr,
                                        guint hwaddr_len,
                                        guint dev_id,
...
/**
 * byx_utils_ipv6_addr_set_interface_identifier:
 ...
 */
void
byx_utils_ipv6_addr_set_interface_identifier (struct in6_addr *addr,
...
/**
 * byx_utils_ipv6_interface_identifier_get_from_addr:
 ...
 */
void
byx_utils_ipv6_interface_identifier_get_from_addr (NMUtilsIPv6IfaceId *iid,
...
/**
 * byx_utils_ipv6_interface_identifier_get_from_token:
 * @iid: output %NMUtilsIPv6IfaceId interface identifier set from the token
 ...
 */
gboolean
byx_utils_ipv6_interface_identifier_get_from_token (NMUtilsIPv6IfaceId *iid,
                                                   const char *token)
{
	...
	if (!_byx_utils_inet6_is_token (&i6_token))
		return FALSE;

	byx_utils_ipv6_interface_identifier_get_from_addr (iid, &i6_token);
	return TRUE;
}

/**
 * byx_utils_inet6_interface_identifier_to_token:
 * @iid: %NMUtilsIPv6IfaceId interface identifier
 ...
 */
const char *
byx_utils_inet6_interface_identifier_to_token (NMUtilsIPv6IfaceId iid, char *buf)
{
	struct in6_addr i6_token = { .s6_addr = { 0, } };

	byx_utils_ipv6_addr_set_interface_identifier (&i6_token, iid);
	return byx_utils_inet6_ntop (&i6_token, buf);
}

/*****************************************************************************/

char *
byx_utils_stable_id_random (void)
{
	char buf[15];

	byx_utils_random_bytes (buf, sizeof (buf));
	return g_base64_encode ((guchar *) buf, sizeof (buf));
}

char *
byx_utils_stable_id_generated_complete (const char *stable_id_generated)
{
	guint8 buf[20];
	GChecksum *sum;
	gsize buf_size;
	 ...
	 * by doing text-substitutions in byx_utils_stable_id_parse().
	 *
...
NMUtilsStableType
byx_utils_stable_id_parse (const char *stable_id,
                          const char *deviceid,
                          const char *bootid,
                          const char *uuid,
                          char **out_generated)
...
#define CHECK_PREFIX(prefix) \
		...
		if (CHECK_PREFIX ("${CONNECTION}"))
			...
			_stable_id_append (str, bootid ?: byx_utils_get_boot_id ());
...
gboolean
byx_utils_ipv6_addr_set_stable_privacy_impl (NMUtilsStableType stable_type,
                                            struct in6_addr *addr,
                                            const char *ifname,
                                            const char *network_id,
                                            guint32 dad_counter,
...
/**
 * byx_utils_ipv6_addr_set_stable_privacy:
 *
 ...
 */
gboolean
byx_utils_ipv6_addr_set_stable_privacy (NMUtilsStableType stable_type,
                                       struct in6_addr *addr,
                                       const char *ifname,
                                       const char *network_id,
                                       guint32 dad_counter,
                                       GError **error)
{
	...
	byx_utils_secret_key_get (&secret_key, &key_len);
                       ...
                       const char *generate_mac_address_mask)
{
	...
	if (!generate_mac_address_mask || !*generate_mac_address_mask)
		goto out;
	if (!_byx_utils_generate_mac_address_mask_parse (generate_mac_address_mask,
	                                                &mask,
	                                                &ouis,
	                                                &ouis_len,
	                                                NULL))
	...
	if (ouis) {
		...
		if (!byx_utils_hwaddr_aton (current_mac_address, &oui, ETH_ALEN))
			goto out;
	}
...
char *
byx_utils_hw_addr_gen_random_eth (const char *current_mac_address,
                                 const char *generate_mac_address_mask)
{
	struct ether_addr bin_addr;

	byx_utils_random_bytes (&bin_addr, ETH_ALEN);
	_hw_addr_eth_complete (&bin_addr, current_mac_address, generate_mac_address_mask);
	return byx_utils_hwaddr_ntoa (&bin_addr, ETH_ALEN);
}
...
char *
byx_utils_hw_addr_gen_stable_eth_impl (NMUtilsStableType stable_type,
                                      const char *stable_id,
                                      const guint8 *secret_key,
                                      gsize key_len,
                                      const char *ifname,
...
char *
byx_utils_hw_addr_gen_stable_eth (NMUtilsStableType stable_type,
                                 const char *stable_id,
                                 const char *ifname,
                                 const char *current_mac_address,
                                 const char *generate_mac_address_mask)
 ...
 */
void
byx_utils_setpgid (gpointer unused G_GNUC_UNUSED)
{
	pid_t pid;
 ...
 */
void
byx_utils_g_value_set_strv (GValue *value, GPtrArray *strings)
{
	char **strv;
	guint i;
...
void
byx_utils_ifname_cpy (char *dst, const char *name)
{
	g_return_if_fail (dst);
	g_return_if_fail (name && name[0]);

	nm_assert (byx_utils_is_valid_iface_name (name, NULL));
...
gboolean
byx_utils_ip4_address_is_link_local (in_addr_t addr)
{
 ...
 */
guint32
byx_utils_lifetime_rebase_relative_time_on_now (guint32 timestamp,
                                               guint32 duration,
                                               gint32 now)
{
	gint64 t;
...
guint32
byx_utils_lifetime_get (guint32 timestamp,
                       guint32 lifetime,
                       guint32 preferred,
                       gint32 now,
                       guint32 *out_preferred)
{
	...
	t_lifetime = byx_utils_lifetime_rebase_relative_time_on_now (timestamp, lifetime, now);
	if (!t_lifetime) {
		NM_SET_OUT (out_preferred, 0);
		return 0;
	}

	t_preferred = byx_utils_lifetime_rebase_relative_time_on_now (timestamp, preferred, now);

	NM_SET_OUT (out_preferred, MIN (t_preferred, t_lifetime));

	/* Assert that non-permanent addresses have a (positive) @timestamp. byx_utils_lifetime_rebase_relative_time_on_now()
...
const char *
byx_utils_dnsmasq_status_to_string (int status, char *dest, gsize size)
{
	const char *msg;

	byx_utils_to_string_buffer_init (&dest, &size);
...
/**
 * byx_utils_get_reverse_dns_domains_ip4:
 ...
 */
void
byx_utils_get_reverse_dns_domains_ip4 (guint32 addr, guint8 plen, GPtrArray *domains)
{
	guint32 ip, ip2, mask;
	guchar *p;
	guint octets;
	...
	while ((ip2 & mask) == ip) {
		...
		str = s = g_malloc (len);
		for (i = octets; i > 0; i--)
			byx_utils_strbuf_append (&s, &len, "%u.", p[i - 1] & 0xff);
		byx_utils_strbuf_append_str (&s, &len, "in-addr.arpa");
...
/**
 * byx_utils_get_reverse_dns_domains_ip6:
 * @addr: IPv6 address
 * @plen: prefix length
 * @domains: array for results
 *
 ...
 */
void
byx_utils_get_reverse_dns_domains_ip6 (const struct in6_addr *ip, guint8 plen, GPtrArray *domains)
{
	struct in6_addr addr;
	...
	byx_utils_ip6_address_clear_host_address (&addr, &addr, plen);
...
#define N_SHIFT(x) ((x) % 2 ? 0 : 4)
	...
	for (i = 0; i < entries; i++) {
		...
		for (j = nibbles - 1; j >= 0; j--)
			byx_utils_strbuf_append (&s,
			                        &len,
			                        "%x.",
			                        (addr.s6_addr[j / 2] >> N_SHIFT (j)) & 0xf);
		byx_utils_strbuf_append_str (&s, &len, "ip6.arpa");
 ...
 */
gboolean
byx_utils_file_set_contents (const gchar *filename,
...
gboolean
byx_utils_validate_plugin (const char *path, struct stat *st, GError **error)
{
	g_return_val_if_fail (path, FALSE);
...
char **
byx_utils_read_plugin_paths (const char *dirname, const char *prefix)
{
	GDir *dir;
	GError *error = NULL;
	const char *item;
	...
	if (!dir) {
		byx_log_warn (LOGD_CORE, "device plugin: failed to open directory %s: %s",
		             dirname,
		             error->message);
		g_clear_error (&error);
		return NULL;
	...
	while ((item = g_dir_read_name (dir))) {
		...
		if (stat (data.path, &data.st) != 0) {
			errsv = errno;
			byx_log_warn (LOGD_CORE,
			             "plugin: skip invalid file %s (error during stat: %s)",
			             data.path, strerror (errsv));
			goto skip;
		}

		if (!byx_utils_validate_plugin (data.path, &data.st, &error)) {
			byx_log_warn (LOGD_CORE,
...
char *
byx_utils_format_con_diff_for_audit (GHashTable *diff)
{
	GHashTable *setting_diff;
...
const char *
byx_utils_parse_dns_domain (const char *domain, gboolean *is_routing)
{
...
GVariant *
byx_utils_strdict_to_variant (GHashTable *options)
{
	...
	keys = byx_utils_strdict_get_keys (options, TRUE, &nkeys);

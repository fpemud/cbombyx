...
void (*_byx_logging_clear_platform_logging_cache) (void);
...
NMLogDomain _byx_logging_enabled_state[_LOGL_N_REAL] = {
	/* byx_logging_setup ("INFO", LOGD_DEFAULT_STRING, NULL, NULL);
	 *
...
void
byx_logging_set_syslog_identifier (const char *domain)
{
                 ...
                 GError     **error)
{
	...
	g_set_error (error, BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_UNKNOWN_LOG_LEVEL,
	             _("Unknown log level '%s'"), level);
	return FALSE;
}

gboolean
byx_logging_setup (const char  *level,
                  const char  *domains,
                  char       **bad_domains,
                  GError     **error)
{
	GString *unrecognized = NULL;
	NMLogDomain new_logging[G_N_ELEMENTS (_byx_logging_enabled_state)];
	ByxLogLevel new_log_level = global.log_level;
	char **tmp, **iter;
	int i;
	...
	if (level && *level) {
		...
		if (new_log_level == _LOGL_KEEP) {
			...
			for (i = 0; i < G_N_ELEMENTS (new_logging); i++)
				new_logging[i] = _byx_logging_enabled_state[i];
		}
	}
	...
	for (iter = tmp; iter && *iter; iter++) {
		...
		/* Check for compatibility domains */
			...
			if (!bits) {
				if (!bad_domains) {
					g_set_error (error, BYX_MANAGER_ERROR, BYX_MANAGER_ERROR_UNKNOWN_LOG_DOMAIN,
		...
		if (domain_log_level == _LOGL_KEEP) {
			for (i = 0; i < G_N_ELEMENTS (new_logging); i++)
				new_logging[i] = (new_logging[i] & ~bits) | (_byx_logging_enabled_state[i] & bits);
		} else {
			for (i = 0; i < G_N_ELEMENTS (new_logging); i++) {
	...
	had_platform_debug = byx_logging_enabled (LOGL_DEBUG, LOGD_PLATFORM);

	global.log_level = new_log_level;
	for (i = 0; i < G_N_ELEMENTS (new_logging); i++)
		_byx_logging_enabled_state[i] = new_logging[i];

	if (   had_platform_debug
	    && _byx_logging_clear_platform_logging_cache
	    && !byx_logging_enabled (LOGL_DEBUG, LOGD_PLATFORM)) {
...
const char *
byx_logging_level_to_string (void)
{
	return global.level_desc[global.log_level].name;
}

const char *
byx_logging_all_levels_to_string (void)
{
	static GString *str;
...
const char *
byx_logging_domains_to_string (void)
{
...
_domains_to_string (gboolean include_level_override)
{
	...
	for (diter = &global.domain_desc[0]; diter->name; diter++) {
		...
		if (!(diter->num & _byx_logging_enabled_state[LOGL_ERR]))
			continue;
		...
		for (i = 0; i < global.log_level; i++) {
			if (diter->num & _byx_logging_enabled_state[i]) {
				g_string_append_printf (str, ":%s", global.level_desc[i].name);
				break;
			}
		}
		/* Check if it's logging at a higher level than the default. */
		if (!(diter->num & _byx_logging_enabled_state[global.log_level])) {
			for (i = global.log_level + 1; i < G_N_ELEMENTS (_byx_logging_enabled_state); i++) {
				if (diter->num & _byx_logging_enabled_state[i]) {
					g_string_append_printf (str, ":%s", global.level_desc[i].name);
					break;
				}
...
const char *
byx_logging_all_domains_to_string (void)
{
	static GString *str;
 ...
 **/
ByxLogLevel
byx_logging_get_level (NMLogDomain domain)
{
	ByxLogLevel sl = _LOGL_OFF;

	G_STATIC_ASSERT (LOGL_TRACE == 0);
	while (   sl > LOGL_TRACE
	       && byx_logging_enabled (sl - 1, domain))
		sl--;
	return sl;
}
              ...
              ...)
{
	...
	if ((guint) level >= G_N_ELEMENTS (_byx_logging_enabled_state))
		g_return_if_reached ();

	if (!(_byx_logging_enabled_state[level] & domain))
		return;
...
#define _NUM_MAX_FIELDS_SYSLOG_FACILITY 10
			...
			now = byx_utils_get_monotonic_timestamp_ns ();
			...
			_iovec_set_format_a (iov++, 30, "SYSLOG_PID=%ld", (long) getpid ());
			{
				...
				NMLogDomain dom = dom_all & _byx_logging_enabled_state[level];
...
void
byx_logging_set_prefix (const char *format, ...)
{
	char *prefix;
	va_list ap;
...
byx_logging_syslog_openlog (const char *logging_backend, gboolean debug)
{
	...
	if (fetch_monotonic_timestamp) {
		 ...
		 * time causes a logging message. We don't want to do that during _byx_log_impl. */
	...
	if (nm_streq (logging_backend, NM_LOG_CONFIG_BACKEND_SYSLOG)) {
		...
		byx_log_warn (LOGD_CORE, "config: invalid logging backend '%s', fallback to '%s'",
		             logging_backend,
#if SYSTEMD_JOURNAL
		             NM_LOG_CONFIG_BACKEND_JOURNAL
#else

...
/* we require that @path is the first member of ByxDBusManagerData
 * because _objects_by_path_hash() requires that. */
G_STATIC_ASSERT (G_STRUCT_OFFSET (struct _ByxDBusObjectInternal, path) == 0);
...
static GVariantBuilder *_obj_collect_properties_all (ByxDBusObject *obj,
...
void
byx_dbus_manager_private_server_register (ByxDBusManager *self,
                                         const char *path,
                                         const char *tag)
{
...
static gboolean
_bus_get_unix_pid (ByxDBusManager *self,
                   const char *sender,
                   gulong *out_pid,
                   GError **error)
{
	...
	ret = _nm_dbus_proxy_call_sync (BYX_DBUS_MANAGER_GET_PRIVATE (self)->proxy,
...
static gboolean
_bus_get_unix_user (ByxDBusManager *self,
                    const char *sender,
                    gulong *out_user,
                    GError **error)
{
 ...
 */
static gboolean
_get_caller_info (ByxDBusManager *self,
                  ...
                  gulong *out_pid)
{
	ByxDBusManagerPrivate *priv = BYX_DBUS_MANAGER_GET_PRIVATE (self);
	const char *sender;
...
gboolean
byx_dbus_manager_get_caller_info (ByxDBusManager *self,
                                 GDBusMethodInvocation *context,
                                 char **out_sender,
                                 gulong *out_uid,
                                 gulong *out_pid)
...
gboolean
byx_dbus_manager_get_caller_info_from_message (ByxDBusManager *self,
                                              GDBusConnection *connection,
                                              GDBusMessage *message,
                                              char **out_sender,
                                              gulong *out_uid,
 ...
 */
gboolean
byx_dbus_manager_ensure_uid (ByxDBusManager          *self,
                            GDBusMethodInvocation *context,
                            gulong uid,
                            GQuark error_domain,
                            int error_code)
{
	...
	g_return_val_if_fail (BYX_IS_DBUS_MANAGER (self), FALSE);
	g_return_val_if_fail (G_IS_DBUS_METHOD_INVOCATION (context), FALSE);

	if (!byx_dbus_manager_get_caller_info (self, context, NULL, &caller_uid, NULL)) {
...
gboolean
byx_dbus_manager_get_unix_user (ByxDBusManager *self,
                               const char *sender,
                               gulong *out_uid)
{
	ByxDBusManagerPrivate *priv = BYX_DBUS_MANAGER_GET_PRIVATE (self);
...
const char *
byx_dbus_manager_connection_get_private_name (ByxDBusManager *self,
                                             GDBusConnection *connection)
{
	ByxDBusManagerPrivate *priv;
	PrivateServer *s;
...
/**
 * byx_dbus_manager_new_proxy:
 * @self: the #ByxDBusManager
 ...
 */
GDBusProxy *
byx_dbus_manager_new_proxy (ByxDBusManager *self,
                           GDBusConnection *connection,
                           GType proxy_type,
                           const char *name,
                           const char *path,
                           const char *iface)
{
	...
	owner = byx_dbus_manager_connection_get_private_name (self, connection);
	if (owner) {
...
GDBusConnection *
byx_dbus_manager_get_connection (ByxDBusManager *self)
{
	g_return_val_if_fail (BYX_IS_DBUS_MANAGER (self), NULL);

	return BYX_DBUS_MANAGER_GET_PRIVATE (self)->connection;
}

/*****************************************************************************/

static const ByxDBusInterfaceInfoExtended *
                         ...
                         gpointer user_data)
{
	...
	ByxDBusObject *obj = reg_data->obj;
	const ByxDBusInterfaceInfoExtended *interface_info = _reg_data_get_interface_info (reg_data);
                   ...
                   gboolean refetch)
{
	...
	property_info = (const ByxDBusPropertyInfoExtended *) (interface_info->parent.properties[property_idx]);
...
static void
_obj_register (ByxDBusManager *self,
               ByxDBusObject *obj)
{
	ByxDBusManagerPrivate *priv = BYX_DBUS_MANAGER_GET_PRIVATE (self);
	guint i, k;
	guint n_klasses;
	GType gtype;
	ByxDBusObjectClass *klasses[10];
	const ByxDBusInterfaceInfoExtended *const*prev_interface_infos = NULL;
	GVariantBuilder builder;
	...
	for (k = n_klasses; k > 0; ) {
		ByxDBusObjectClass *klass = NM_DBUS_OBJECT_CLASS (klasses[--k]);
...
static void
_obj_unregister (ByxDBusManager *self,
                 ByxDBusObject *obj)
{
	ByxDBusManagerPrivate *priv = BYX_DBUS_MANAGER_GET_PRIVATE (self);
	RegistrationData *reg_data;
...
ByxDBusObject *
byx_dbus_manager_lookup_object (ByxDBusManager *self, const char *path)
{
	ByxDBusManagerPrivate *priv;
	gpointer ptr;
	ByxDBusObject *obj;
	...
	obj = (ByxDBusObject *) (((char *) ptr) - G_STRUCT_OFFSET (ByxDBusObject, internal));
	nm_assert (NM_IS_DBUS_OBJECT (obj));
	return obj;
}

void
_byx_dbus_manager_obj_export (ByxDBusObject *obj)
{
	ByxDBusManager *self;
	ByxDBusManagerPrivate *priv;
	...
	g_return_if_fail (BYX_IS_DBUS_MANAGER (obj->internal.bus_manager));
...
void
_byx_dbus_manager_obj_unexport (ByxDBusObject *obj)
{
	ByxDBusManager *self;
	ByxDBusManagerPrivate *priv;
...
void
_byx_dbus_manager_obj_notify (ByxDBusObject *obj,
                             guint n_pspecs,
                             const GParamSpec *const*pspecs)
{
	...
	nm_assert (BYX_IS_DBUS_MANAGER (obj->internal.bus_manager));
	...
	/* do a naive search for the matching ByxDBusPropertyInfoExtended infos. Since the number of
	...
	c_list_for_each_entry (reg_data, &obj->internal.registration_lst_head, registration_lst) {
		...
		for (i = 0; interface_info->parent.properties[i]; i++) {
			const ByxDBusPropertyInfoExtended *property_info = (const ByxDBusPropertyInfoExtended *) interface_info->parent.properties[i];
	...
	if (any_legacy_properties) {
		...
		/* The legacy PropertyChanged signal on the Bombyx D-Bus interface is
		 * deprecated for the standard signal on org.fpemud.DBus.Properties. However,
		 * for backward compatibility, we still need to emit it.
		 *
		 * Due to a bug in dbus-glib in Bombyx <= 1.0, the signal would
...
void
_byx_dbus_manager_obj_emit_signal (ByxDBusObject *obj,
...
_obj_collect_properties_per_interface (ByxDBusObject *obj,
                                       RegistrationData *reg_data,
                                       GVariantBuilder *builder)
{
	const ByxDBusInterfaceInfoExtended *interface_info = _reg_data_get_interface_info (reg_data);
...
static GVariantBuilder *
_obj_collect_properties_all (ByxDBusObject *obj,
                             GVariantBuilder *builder)
{
	RegistrationData *reg_data;
...
byx_dbus_manager_acquire_bus (ByxDBusManager *self)
{
	 ...
	 *    it means that something is borked. Quite possibly another Bombyx
	 *    instance is running. We need to exit right away.
	 * To appease (1) and (3), we cannot initalize synchronously, because we need
	 * to know right away whether another Bombyx instance is running (3).
	 **/
...
void
byx_dbus_manager_stop (ByxDBusManager *self)
{
	ByxDBusManagerPrivate *priv = BYX_DBUS_MANAGER_GET_PRIVATE (self);
...
gboolean
byx_dbus_manager_is_stopping (ByxDBusManager *self)
{
	return BYX_DBUS_MANAGER_GET_PRIVATE (self)->shutting_down;
}

/*****************************************************************************/

static void
BYX_DBUS_MANAGER_class_init (ByxDBusManagerClass *klass)
{
...
static void
BYX_DBUS_MANAGER_init (ByxDBusManager *self)
{
	ByxDBusManagerPrivate *priv = BYX_DBUS_MANAGER_GET_PRIVATE (self);
...
{
	ByxDBusManager *self = BYX_DBUS_MANAGER (object);
	ByxDBusManagerPrivate *priv = BYX_DBUS_MANAGER_GET_PRIVATE (self);
	PrivateServer *s, *s_safe;

	/* All exported ByxDBusObject instances keep the manager alive, so we don't
	...
	G_OBJECT_CLASS (BYX_DBUS_MANAGER_parent_class)->dispose (object);
}

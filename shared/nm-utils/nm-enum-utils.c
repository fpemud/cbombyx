                              ...
                              const NMUtilsEnumValueInfo *value_infos)
{
	...
	if (G_IS_ENUM_CLASS (klass)) {
		...
		if (s[0]) {
			if (_is_hex_string (s)) {
				v64 = _byx_utils_ascii_str_to_int64 (s, 16, 0, G_MAXUINT, -1);
				if (v64 != -1) {
					value = (int) v64;
					ret = TRUE;
				}
			} else if (_is_dec_string (s)) {
				v64 = _byx_utils_ascii_str_to_int64 (s, 10, 0, G_MAXUINT, -1);
				if (v64 != -1) {
					value = (int) v64;
					ret = TRUE;
				}
		...
		while (s[0]) {
			...
			if (s[0]) {
				if (_is_hex_string (s)) {
					v64 = _byx_utils_ascii_str_to_int64 (&s[2], 16, 0, G_MAXUINT, -1);
					if (v64 == -1) {
						ret = FALSE;
						break;
					}

...
__nmtst_init (int *argc, char ***argv, gboolean assert_logging, const char *log_level, const char *log_domains, gboolean *out_set_logging)
{
	...
	*out_set_logging = FALSE;
	...
	if (!byx_utils_get_testing_initialized ())
		_byx_utils_set_testing (_NM_UTILS_TEST_GENERAL);
#endif
	...
	if (!__nmtst_internal.assert_logging) {
		...
		success = byx_logging_setup (log_level, log_domains, NULL, NULL);
...
#define NMTST_UUID_INIT(uuid) \
	gs_free char *_nmtst_hidden_##uuid = byx_utils_uuid_generate (); \
                                     ...
                                     guint prefix)
{
	...
	if (byx_utils_ipaddr_valid (AF_INET, address))
		family = AF_INET;
	else if (byx_utils_ipaddr_valid (AF_INET6, address))
		family = AF_INET6;
	else
		g_assert_not_reached ();
                                   ...
                                   gint64 metric)
{
	...
	if (byx_utils_ipaddr_valid (AF_INET, dest))
		family = AF_INET;
	else if (byx_utils_ipaddr_valid (AF_INET6, dest))
		family = AF_INET6;
	else
		g_assert_not_reached ();
...
nmtst_create_minimal_connection (const char *id, const char *uuid, const char *type, NMSettingConnection **out_s_con)
{
	...
	if (uuid)
		g_assert (byx_utils_is_uuid (uuid));
	else
		uuid = uuid_free = byx_utils_uuid_generate ();
...
nmtst_assert_hwaddr_equals (gconstpointer hwaddr1, gssize hwaddr1_len, const char *expected, const char *file, int line)
{
	...
	g_assert (byx_utils_hwaddr_aton (expected, buf2, hwaddr2_len));

	/* Manually check the entire hardware address instead of using
	 * byx_utils_hwaddr_matches() because that function doesn't compare
	...
	if (!success) {
		g_error ("assert: %s:%d: hwaddr '%s' (%zd) expected, but got %s (%zd)",
		         file, line, expected, hwaddr2_len, byx_utils_hwaddr_ntoa (hwaddr1, hwaddr1_len), hwaddr1_len);
	}
}

...
static inline int
byx_utils_syslog_coerce_from_nm (int syslog_level)
{
	/* NetworkManager uses internally ByxLogLevel levels. When spawning
	 ...
	 *   syslog(syslog_level, ...)
	 * you should call
	 *   syslog(byx_utils_syslog_coerce_from_nm(syslog_level), ...)
	 */
	switch (syslog_level) {
	case LOG_INFO:
		return LOG_DEBUG;
...
byx_utils_syslog_to_str (int syslog_level)
{

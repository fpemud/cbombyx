...
void
byx_utils_strbuf_append_c (char **buf, gsize *len, char c)
{
	switch (*len) {
	case 0:
		return;
...
void
byx_utils_strbuf_append_str (char **buf, gsize *len, const char *str)
{
	gsize src_len;
...
void
byx_utils_strbuf_append (char **buf, gsize *len, const char *format, ...)
{
	char *p = *buf;
	va_list args;
	gint retval;
...
nm_strquote (char *buf, gsize buf_len, const char *str)
{
	...
	if (!str) {
		byx_utils_strbuf_append_str (&buf, &buf_len, "(null)");
		goto out;
	}
	...
	*(buf++) = '"';
	...
	byx_utils_strbuf_append_str (&buf, &buf_len, str);
...
char _byx_utils_to_string_buffer[];

void
byx_utils_to_string_buffer_init (char **buf, gsize *len)
{
	if (!*buf) {
		*buf = _byx_utils_to_string_buffer;
		*len = sizeof (_byx_utils_to_string_buffer);
	}
}

gboolean
byx_utils_to_string_buffer_init_null (gconstpointer obj, char **buf, gsize *len)
{
...
const char *
byx_utils_flags2str (const NMUtilsFlags2StrDesc *descs,
                    gsize n_descs,
                    unsigned flags,
                    char *buf,
                    gsize len)
{
	...
	if (!flags) {
		for (i = 0; i < n_descs; i++) {
			if (!descs[i].flag) {
				byx_utils_strbuf_append_str (&p, &len, descs[i].name);
				break;
			}
		}
		return buf;
	...
	for (i = 0; flags && i < n_descs; i++) {
		if (   descs[i].flag
			...
			if (buf[0] != '\0')
				byx_utils_strbuf_append_c (&p, &len, ',');
			byx_utils_strbuf_append_str (&p, &len, descs[i].name);
		}
	}
	if (flags) {
		if (buf[0] != '\0')
			byx_utils_strbuf_append_c (&p, &len, ',');
		byx_utils_strbuf_append (&p, &len, "0x%x", flags);
	}
	return buf;
};
...
/**
 * _byx_utils_ip4_prefix_to_netmask:
 * @prefix: a CIDR prefix
 *
 * Returns: the netmask represented by the prefix, in network byte order
 **/
guint32
_byx_utils_ip4_prefix_to_netmask (guint32 prefix)
{
	return prefix < 32 ? ~htonl(0xFFFFFFFF >> prefix) : 0xFFFFFFFF;
}

/**
 * _byx_utils_ip4_get_default_prefix:
...
guint32
_byx_utils_ip4_get_default_prefix (guint32 ip)
{
...
gboolean
byx_utils_ip_is_site_local (int addr_family,
                           const void *address)
{
	in_addr_t addr4;
...
gboolean
byx_utils_parse_inaddr_bin (int addr_family,
                           const char *text,
                           gpointer out_addr)
{
	NMIPAddr addrbin;
...
gboolean
byx_utils_parse_inaddr (int addr_family,
                       const char *text,
                       char **out_addr)
{
	NMIPAddr addrbin;
	...
	if (!byx_utils_parse_inaddr_bin (addr_family, text, &addrbin))
		return FALSE;
...
gboolean
byx_utils_parse_inaddr_prefix_bin (int addr_family,
                                  const char *text,
                                  gpointer out_addr,
                                  int *out_prefix)
{
	...
	if (slash) {
		prefix = _byx_utils_ascii_str_to_int64 (slash + 1, 10,
		                                       0,
...
gboolean
byx_utils_parse_inaddr_prefix (int addr_family,
                              const char *text,
                              char **out_addr,
                              int *out_prefix)
{
	...
	if (!byx_utils_parse_inaddr_prefix_bin (addr_family, text, &addrbin, out_prefix))
		return FALSE;
	NM_SET_OUT (out_addr, g_strdup (inet_ntop (addr_family, &addrbin, addrstr_buf, sizeof (addrstr_buf))));
	return TRUE;
}
 ...
 **/
gint64
_byx_utils_ascii_str_to_int64 (const char *str, guint base, gint64 min, gint64 max, gint64 fallback)
{
	gint64 v;
	const char *s = NULL;
...
const char *
byx_utils_dbus_path_get_last_component (const char *dbus_path)
{
	if (dbus_path) {
...
int
byx_utils_dbus_path_cmp (const char *dbus_path_a, const char *dbus_path_b)
{
	const char *l_a, *l_b;
	gsize plen;
	gint64 n_a, n_b;
	...
	if (   !(l_a = byx_utils_dbus_path_get_last_component (dbus_path_a))
	    || !(l_b = byx_utils_dbus_path_get_last_component (dbus_path_b)))
		goto comp_full;
 ...
 */
const char **
byx_utils_strsplit_set (const char *str, const char *delimiters)
{
	const char **ptr, **ptr0;
 ...
 */
gssize
byx_utils_strv_find_first (char **list, gssize len, const char *needle)
{
	gssize i;
...
char **
_byx_utils_strv_cleanup (char **strv,
                        ...
                        gboolean skip_repeated)
{
	...
	for (i = 0; strv[i]; i++) {
		if (   (skip_empty && !*strv[i])
		    || (skip_repeated && byx_utils_strv_find_first (strv, j, strv[i]) >= 0))
			g_free (strv[i]);
		else
			strv[j++] = strv[i];
	}
...
gint
_byx_utils_ascii_str_to_bool (const char *str,
                             gint default_value)
{
	gsize len;
	char *s = NULL;
...
NM_CACHED_QUARK_FCN ("nm-utils-error-quark", byx_utils_error_quark)

void
byx_utils_error_set_cancelled (GError **error,
...
gboolean
byx_utils_error_is_cancelled (GError *error,
 ...
 * byx_utils_str_utf8safe_unescape().
 *
 ...
 **/
const char *
byx_utils_str_utf8safe_escape (const char *str, NMUtilsStrUtf8SafeFlags flags, char **to_free)
{
	const char *p = NULL;
	GString *s;
...
const char *
byx_utils_str_utf8safe_unescape (const char *str, char **to_free)
{
	g_return_val_if_fail (to_free, NULL);
...
/**
 * byx_utils_str_utf8safe_escape_cp:
 * @str: NUL terminated input string, possibly in utf-8 encoding
 * @flags: #NMUtilsStrUtf8SafeFlags flags
 *
 * Like byx_utils_str_utf8safe_escape(), except the returned value
 ...
 **/
char *
byx_utils_str_utf8safe_escape_cp (const char *str, NMUtilsStrUtf8SafeFlags flags)
{
	char *s;

	byx_utils_str_utf8safe_escape (str, flags, &s);
	return s ?: g_strdup (str);
}

char *
byx_utils_str_utf8safe_unescape_cp (const char *str)
{
	return str ? g_strcompress (str) : NULL;
}

char *
byx_utils_str_utf8safe_escape_take (char *str, NMUtilsStrUtf8SafeFlags flags)
{
	char *str_to_free;

	byx_utils_str_utf8safe_escape (str, flags, &str_to_free);
	if (str_to_free) {
		g_free (str);
		return str_to_free;
	}
...
int
byx_utils_fd_wait_for_event (int fd, int event, gint64 timeout_ns)
{
...
ssize_t
byx_utils_fd_read_loop (int fd, void *buf, size_t nbytes, bool do_poll)
{
	uint8_t *p = buf;
	ssize_t n = 0;
	...
	do {
		...
		if (k < 0) {
			...
			if (errno == EAGAIN && do_poll) {
				...
				(void) byx_utils_fd_wait_for_event (fd, POLLIN, -1);
				continue;
			}
...
int
byx_utils_fd_read_loop_exact (int fd, void *buf, size_t nbytes, bool do_poll)
{
	ssize_t n;

	n = byx_utils_fd_read_loop (fd, buf, nbytes, do_poll);
	if (n < 0)
		return (int) n;
	if ((size_t) n != nbytes)
		return -EIO;
...
NMUtilsNamedValue *
byx_utils_named_values_from_str_dict (GHashTable *hash, guint *out_len)
{
	GHashTableIter iter;
	...
	if (len > 1) {
		g_qsort_with_data (values, len, sizeof (values[0]),
		                   byx_utils_named_entry_cmp_with_data, NULL);
	}
...
gpointer *
byx_utils_hash_keys_to_array (GHashTable *hash,
...
char **
byx_utils_strv_make_deep_copied (const char **strv)
{
	gsize i;
...
/**
 * byx_utils_get_start_time_for_pid:
 ...
 **/
guint64
byx_utils_get_start_time_for_pid (pid_t pid, char *out_state, pid_t *out_ppid)
{
	guint64 start_time;
	char filename[256];
	...
	tokens = byx_utils_strsplit_set (p, " ");

	if (NM_PTRARRAY_LEN (tokens) < 20)
		goto fail;

	if (out_ppid) {
		ppid = _byx_utils_ascii_str_to_int64 (tokens[1], 10, 1, G_MAXINT, 0);
		if (ppid == 0)
			goto fail;
	}

	start_time = _byx_utils_ascii_str_to_int64 (tokens[19], 10, 1, G_MAXINT64, 0);
	if (start_time == 0)
		goto fail;
 ...
 */
void
_byx_utils_strv_sort (const char **strv, gssize len)
{
	gsize l;
...
gpointer
_byx_utils_user_data_pack (int nargs, gconstpointer *args)
{
	int i;
	gpointer *data;
...
void
_byx_utils_user_data_unpack (gpointer user_data, int nargs, ...)
{

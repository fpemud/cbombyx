...
static inline char
byx_utils_addr_family_to_char (int addr_family)
{
	switch (addr_family) {
...
static inline gsize
byx_utils_addr_family_to_size (int addr_family)
{
	switch (addr_family) {
...
static inline void
_byx_utils_strbuf_init (char *buf, gsize len, char **p_buf_ptr, gsize *p_buf_len)
{
...
#define byx_utils_strbuf_init(buf, p_buf_ptr, p_buf_len) \
	G_STMT_START { \
		G_STATIC_ASSERT (G_N_ELEMENTS (buf) == sizeof (buf) && sizeof (buf) > sizeof (char *)); \
		_byx_utils_strbuf_init ((buf), sizeof (buf), (p_buf_ptr), (p_buf_len)); \
	} G_STMT_END
void byx_utils_strbuf_append (char **buf, gsize *len, const char *format, ...) _nm_printf (3, 4);
void byx_utils_strbuf_append_c (char **buf, gsize *len, char c);
void byx_utils_strbuf_append_str (char **buf, gsize *len, const char *str);

const char *nm_strquote (char *buf, gsize buf_len, const char *str);

static inline gboolean
byx_utils_is_separator (const char c)
{
	return NM_IN_SET (c, ' ', '\t');
}
...
const char *byx_utils_dbus_path_get_last_component (const char *dbus_path);

int byx_utils_dbus_path_cmp (const char *dbus_path_a, const char *dbus_path_b);

/*****************************************************************************/

const char **byx_utils_strsplit_set (const char *str, const char *delimiters);

gssize byx_utils_strv_find_first (char **list, gssize len, const char *needle);

char **_byx_utils_strv_cleanup (char **strv,
...
guint32 _byx_utils_ip4_prefix_to_netmask (guint32 prefix);
guint32 _byx_utils_ip4_get_default_prefix (guint32 ip);

gboolean byx_utils_ip_is_site_local (int addr_family,
                                    const void *address);

/*****************************************************************************/

gboolean byx_utils_parse_inaddr_bin  (int addr_family,
                                     const char *text,
                                     gpointer out_addr);

gboolean byx_utils_parse_inaddr (int addr_family,
                                const char *text,
                                char **out_addr);

gboolean byx_utils_parse_inaddr_prefix_bin (int addr_family,
                                           const char *text,
                                           gpointer out_addr,
                                           int *out_prefix);

gboolean byx_utils_parse_inaddr_prefix (int addr_family,
                                       const char *text,
                                       char **out_addr,
                                       int *out_prefix);

gint64 _byx_utils_ascii_str_to_int64 (const char *str, guint base, gint64 min, gint64 max, gint64 fallback);

gint _byx_utils_ascii_str_to_bool (const char *str,
                                  gint default_value);

/*****************************************************************************/

extern char _byx_utils_to_string_buffer[2096];

void     byx_utils_to_string_buffer_init (char **buf, gsize *len);
gboolean byx_utils_to_string_buffer_init_null (gconstpointer obj, char **buf, gsize *len);
...
{ \
	...
	return byx_utils_flags2str (descs, G_N_ELEMENTS (descs), flags, buf, len); \
};
...
const char *byx_utils_flags2str (const NMUtilsFlags2StrDesc *descs,
                                gsize n_descs,
                                unsigned flags,
                                char *buf,
                                gsize len);
...
#define NM_UTILS_ERROR (byx_utils_error_quark ())
GQuark byx_utils_error_quark (void);

void byx_utils_error_set_cancelled (GError **error,
                                   gboolean is_disposing,
                                   const char *instance_name);
gboolean byx_utils_error_is_cancelled (GError *error,
...
const char *byx_utils_str_utf8safe_escape   (const char *str, NMUtilsStrUtf8SafeFlags flags, char **to_free);
const char *byx_utils_str_utf8safe_unescape (const char *str, char **to_free);

char *byx_utils_str_utf8safe_escape_cp   (const char *str, NMUtilsStrUtf8SafeFlags flags);
char *byx_utils_str_utf8safe_unescape_cp (const char *str);

char *byx_utils_str_utf8safe_escape_take (char *str, NMUtilsStrUtf8SafeFlags flags);
...
#define byx_utils_named_entry_cmp           nm_strcmp_p
#define byx_utils_named_entry_cmp_with_data nm_strcmp_p_with_data

NMUtilsNamedValue *byx_utils_named_values_from_str_dict (GHashTable *hash, guint *out_len);

gpointer *byx_utils_hash_keys_to_array (GHashTable *hash,
                                       GCompareDataFunc compare_func,
                                       gpointer user_data,
                                       guint *out_len);

static inline const char **
byx_utils_strdict_get_keys (const GHashTable *hash,
                           gboolean sorted,
                           guint *out_length)
{
	return (const char **) byx_utils_hash_keys_to_array ((GHashTable *) hash,
	                                                    sorted ? nm_strcmp_p_with_data : NULL,
	                                                    NULL,
	                                                    out_length);
}

char **byx_utils_strv_make_deep_copied (const char **strv);

static inline char **
byx_utils_strv_make_deep_copied_nonnull (const char **strv)
{
	return byx_utils_strv_make_deep_copied (strv) ?: g_new0 (char *, 1);
}

void _byx_utils_strv_sort (const char **strv, gssize len);
#define byx_utils_strv_sort(strv, len) _byx_utils_strv_sort (NM_CAST_STRV_MC (strv), len)
...
int byx_utils_fd_wait_for_event (int fd, int event, gint64 timeout_ns);
ssize_t byx_utils_fd_read_loop (int fd, void *buf, size_t nbytes, bool do_poll);
int byx_utils_fd_read_loop_exact (int fd, void *buf, size_t nbytes, bool do_poll);

/*****************************************************************************/

static inline const char *
byx_utils_dbus_normalize_object_path (const char *path)
{
...
guint64 byx_utils_get_start_time_for_pid (pid_t pid, char *out_state, pid_t *out_ppid);

/*****************************************************************************/

gpointer _byx_utils_user_data_pack (int nargs, gconstpointer *args);

#define byx_utils_user_data_pack(...) \
	_byx_utils_user_data_pack(NM_NARG (__VA_ARGS__), (gconstpointer[]) { __VA_ARGS__ })

void _byx_utils_user_data_unpack (gpointer user_data, int nargs, ...);

#define byx_utils_user_data_unpack(user_data, ...) \
	_byx_utils_user_data_unpack(user_data, NM_NARG (__VA_ARGS__), __VA_ARGS__)

...
test_read_valid_wired_connection (void)
{
	...
	g_assert (mac);
	g_assert (byx_utils_hwaddr_matches (mac, -1, expected_mac_address, sizeof (expected_mac_address)));
...
test_read_mac_old_format (void)
{
	...
	mac = nm_setting_wired_get_mac_address (s_wired);
	g_assert (mac);
	g_assert (byx_utils_hwaddr_matches (mac, -1, expected_mac, ETH_ALEN));

	/* Cloned MAC address */
	mac = nm_setting_wired_get_cloned_mac_address (s_wired);
	g_assert (mac);
	g_assert (byx_utils_hwaddr_matches (mac, -1, expected_cloned_mac, ETH_ALEN));
}
...
test_read_mac_ib_old_format (void)
{
	...
	g_assert (mac);
	g_assert (byx_utils_hwaddr_matches (mac, -1, expected_mac, sizeof (expected_mac)));
}
...
test_read_valid_wireless_connection (void)
{
	...
	g_assert (bssid);
	g_assert (byx_utils_hwaddr_matches (bssid, -1, expected_bssid, sizeof (expected_bssid)));
...
test_read_bt_dun_connection (void)
{
	...
	g_assert (bdaddr);
	g_assert (byx_utils_hwaddr_matches (bdaddr, -1, expected_bdaddr, sizeof (expected_bdaddr)));
...
{
	_byx_utils_set_testing (NM_UTILS_TEST_NO_KEYFILE_OWNER_CHECK);

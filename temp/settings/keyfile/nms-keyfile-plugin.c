...
#define NMS_KEYFILE_PLUGIN_GET_PRIVATE(self) _BYX_GET_PRIVATE (self, NMSKeyfilePlugin, NMS_IS_KEYFILE_PLUGIN)
...
get_connections (NMSettingsPlugin *config)
{
	...
	return _byx_utils_hash_values_to_slist (priv->connections);
}
...
get_unmanaged_specs (NMSettingsPlugin *config)
{
	...
	value = byx_config_data_get_value (byx_config_manager_get_data (priv->config),

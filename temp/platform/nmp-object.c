...
{
	const NMPClass *klass;
	char buf2[sizeof (_byx_utils_to_string_buffer)];

	if (!byx_utils_to_string_buffer_init_null (obj, &buf, &buf_size))
		return buf;
...
_vt_cmd_obj_to_string_link (const NMPObject *obj, NMPObjectToStringMode to_string_mode, char *buf, gsize buf_size)
{
	...
	char buf2[sizeof (_byx_utils_to_string_buffer)];
	char buf3[sizeof (_byx_utils_to_string_buffer)];
...
_vt_cmd_plobj_to_string_id (ip4_address, NMPlatformIP4Address, "%d: %s/%d%s%s", obj->ifindex, byx_utils_inet4_ntop ( obj->address, buf1), obj->plen,
                                                               obj->peer_address != obj->address ? "," : "",
                                                               obj->peer_address != obj->address ? byx_utils_inet4_ntop (byx_utils_ip4_address_clear_host_address (obj->peer_address, obj->plen), buf2) : "");
_vt_cmd_plobj_to_string_id (ip6_address, NMPlatformIP6Address, "%d: %s",        obj->ifindex, byx_utils_inet6_ntop (&obj->address, buf1));
...
_vt_cmd_plobj_id_hash_update (ip4_address, NMPlatformIP4Address, {
	nm_hash_update_vals (h,
	                     ...
	                     byx_utils_ip4_address_clear_host_address (obj->peer_address, obj->plen));
})
